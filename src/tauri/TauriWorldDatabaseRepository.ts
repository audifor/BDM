...
import { parseWorldCompetitionRuntimeBundle, type WorldCompetitionRuntimeBundle } from '@/domain/competition'
...
export const tauriWorldDatabaseRepository: WorldDatabaseRepository = {
  ...
  async loadCompetitionRuntimeBundle(bundlePath) {
    const value = await invoke<unknown>('load_world_db_competition_runtime_bundle_v1', { bundlePath })
