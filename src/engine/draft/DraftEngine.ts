...
export function chooseAiDraftProspect(world: GameWorld, draftId: string, organizationTeamId?: TeamId): PlayerId | undefined { const teamId=organizationTeamId??getCurrentDraftPick(world,draftId)?.ownerTeamId;if(!teamId)return undefined;const organizationId=world.teams[teamId]!.organizationId,policy=world.organizationEvaluationPoliciesById[organizationId];return getAvailableDraftProspects(world, draftId).slice().sort((a,b)=>{const first=world.players[a]!,second=world.players[b]!;const firstValue=deriveOrganizationPlayerValuation({organizationId,playerId:a,knowledge:world.organizationKnowledge,currentDate:world.currentDate,context:'DRAFT',publicPosition:first.basketball.primaryPosition,policy});const secondValue=deriveOrganizationPlayerValuation({organizationId,playerId:b,knowledge:world.organizationKnowledge,currentDate:world.currentDate,context:'DRAFT',publicPosition:second.basketball.primaryPosition,policy});return secondValue.priorityScore-firstValue.priorityScore||a.localeCompare(b)})[0] }
