...
describe('WorldDbScheduleResolver', () => {
  it('resolves historical and current allocation/timing rows by validity interval without interpreting statuses', () => {
    ...
    expect(before['fixture:1']).toMatchObject({ allocationId: 'allocation:old', allocationStatus: 'SUPERSEDED', competitionScheduleSlotId: 'slot:old', timing: { timingHistoryId: 'timing:old', timingState: 'CONFIRMED', localDate: '2026-06-02' } })
    const after = resolveWorldDbFixtureSchedulesAtV1(baseBundle, '2026-05-25T12:00:00Z')
    expect(after['fixture:1']).toMatchObject({ allocationId: 'allocation:new', allocationStatus: 'ACTIVE', competitionScheduleSlotId: 'slot:new', timing: { timingHistoryId: 'timing:new', localDate: '2026-06-04', localTime: '20:30' } })
  })
  ...
  it('lists a local date deterministically by local time, slot order and fixture id', () => {
    ...
    expect(listWorldDbFixturesScheduledOnLocalDateV1(resolved, '2026-06-04').map((row) => row.competitionFixtureId)).toEqual(['fixture:2', 'fixture:1'])
  })

  it('returns null for an unscheduled fixture and permits an allocated slot without active timing', () => {
    const bundle: WorldDbCompetitionBundleV1 = { ...baseBundle, fixtureScheduleAllocations: Object.freeze([]), scheduleSlotTimings: Object.freeze([]) }
