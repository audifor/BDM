      ...
      for (const destination of destinationsByRuleId[rule.id] ?? []) {
        ...
        if (destinationPositionId !== undefined) {
          ...
          if (position.competitionStructureNodeId !== destinationNodeId) {
            throw new Error(
              `Progression destination position ${destinationPositionId} does not belong to node ${destinationNodeId}`,
            )
          }
        }
...
function readDestination(
  destination: WorldDbRuleRecordV1,
): { readonly nodeId: string; readonly positionId?: string } {
