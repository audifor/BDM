...
/** Resolves deterministic bracket participants while preserving original seed identity. */
export function resolveWorldCompetitionVirtualFixturesV1(
  ...
  for (const fixture of plan.fixtures) {
    ...
    if (left.competitionSeasonEntryId === right.competitionSeasonEntryId) throw new Error(`Virtual fixture resolves the same entry on both sides: ${fixture.fixtureId}`)
  ...
  return Object.freeze({ resolvedFixtures: Object.freeze(resolvedFixtures), readyFixtures: Object.freeze(readyFixtures), completedFixtureIds: Object.freeze(completedFixtureIds) })
}
...
function buildSeedIndex(fixtures: readonly WorldCompetitionVirtualFixtureV1[]): ReadonlyMap<string, number> {
  ...
  for (const fixture of fixtures) {
    for (const ref of fixture.participants) {
      ...
      if (previous !== undefined && previous !== ref.seed) throw new Error(`Competition entry has conflicting bracket seeds: ${ref.competitionSeasonEntryId}`)
      for (const [otherEntryId, otherSeed] of result) {
        if (otherEntryId !== ref.competitionSeasonEntryId && otherSeed === ref.seed) throw new Error(`Bracket seed ${ref.seed} is assigned to multiple competition entries`)
      }
...
function resolveParticipant(
  ...
  if (ref.kind === 'ENTRY') return Object.freeze({ competitionSeasonEntryId: ref.competitionSeasonEntryId, seed: ref.seed })
...
function freezeResolvedFixture(
  ...
  return Object.freeze({ fixtureId: fixture.fixtureId, competitionSeasonId: fixture.competitionSeasonId, variantKey: fixture.variantKey, nodeKey: fixture.nodeKey, fixtureOrder: fixture.fixtureOrder, participants: Object.freeze([left, right]) as readonly [WorldCompetitionResolvedParticipantV1, WorldCompetitionResolvedParticipantV1] })
}

function validateOutcomeParticipants(fixture: WorldCompetitionResolvedVirtualFixtureV1, outcome: WorldCompetitionVirtualFixtureOutcomeV1): void {
  const participants = new Set(fixture.participants.map((participant) => participant.competitionSeasonEntryId))
  if (!participants.has(outcome.winnerEntryId) || !participants.has(outcome.loserEntryId)) throw new Error(`Virtual fixture outcome participants do not match resolved fixture: ${fixture.fixtureId}`)
}
