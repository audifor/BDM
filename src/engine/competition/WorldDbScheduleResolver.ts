...
/**
 * Resolves canonical B04 fixture scheduling at one point in time.
 *
 * Validity intervals use inclusive `validFrom` and exclusive `validTo`. Status and timing-state
 * vocabularies are transported unchanged and are not interpreted here. Missing allocation means
 * the fixture is currently unscheduled. Multiple simultaneously active rows are rejected because
 * choosing one would silently discard canonical reschedule history.
 */
