...
function chooseEvaluator(world:GameWorld, organizationId:OrganizationId, mission:ScoutingMission):StaffPersonId { const teamId = organizationId as unknown as TeamId; const candidates = Object.values(world.teamStaffAssignmentsById).filter((item) => item.teamId === teamId && item.role === 'scout').map((item) => item.staffPersonId); const selected = candidates.sort((a,b) => score(world,b,mission)-score(world,a,mission) || a.localeCompare(b))[0]; if (!selected) throw new Error('Organization has no scout'); return selected }
