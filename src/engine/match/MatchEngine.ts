...
export function substitutePlayer(session: MatchSession, substitution: SubstitutePlayerOptions): MatchSession {
  ...
  return { ...session, state: { ...state, activeLineups, spatial, ...(state.offBallCut?.playerId === substitution.playerOutId ? { offBallCut: undefined } : {}), ...(removesScreenParticipant ? { screenIntent: undefined } : {}), nextSequence: state.nextSequence + 1, events: [...state.events, event] } }
}
...
export function stepMatchSession(session: MatchSession): MatchSessionStepResult {
  ...
  const movementProfiles = screenAffectsDefender && screenForMovement !== undefined ? reduceScreenedDefenderMovement(state.playerProfiles, screenForMovement.defenderId) : state.playerProfiles
  ...
  const stateAfterAction = { ...state, clockSecondsRemaining, homeScore, awayScore, attackingTeamId, spatial, offBallCut, screenIntent, passesThisPossession, nextSequence: sequence }
...
function finishPeriod(session: MatchSession, state: MatchSessionState, newEvents: MatchEvent[]): MatchSessionStepResult {
  ...
  if (state.period >= state.clockRules.periodCount && state.homeScore !== state.awayScore) {
    ...
    const completeState = { ...state, spatial: releaseSpatialBall(state.spatial), offBallCut: undefined, screenIntent: undefined, passesThisPossession: 0, nextSequence: sequence + 1, events: [...state.events, ...newEvents], isComplete: true }
  ...
  const nextState = { ...state, period, clockSecondsRemaining, attackingTeamId, offBallCut: undefined, screenIntent: undefined, passesThisPossession: 0, spatial: releaseSpatialBall(state.spatial), nextSequence: sequence, events: [...state.events, ...newEvents] }
