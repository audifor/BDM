...
export function clonePlan(plan: MatchTacticalPlan): MatchTacticalPlan { return { pace: plan.pace, shotProfile: { ...plan.shotProfile }, defense: { ...plan.defense, pickAndRollCoverage: plan.defense.pickAndRollCoverage ?? 'switch' }, ...(plan.featuredPlayerId === undefined ? {} : { featuredPlayerId: plan.featuredPlayerId as PlayerId }) } }
function samePlan(left: MatchTacticalPlan, right: MatchTacticalPlan): boolean { return left.pace === right.pace && left.shotProfile.rim === right.shotProfile.rim && left.shotProfile.midRange === right.shotProfile.midRange && left.shotProfile.threePoint === right.shotProfile.threePoint && left.defense.interior === right.defense.interior && left.defense.perimeter === right.defense.perimeter && (left.defense.pickAndRollCoverage ?? 'switch') === (right.defense.pickAndRollCoverage ?? 'switch') && left.featuredPlayerId === right.featuredPlayerId }
