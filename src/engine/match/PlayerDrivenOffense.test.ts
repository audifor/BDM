...
describe('player-driven offense primitives', () => {
  ...
  it('derives shot-zone weights, points, and make probability from profile, fatigue, and defense proxy', () => {
    ...
    expect(calculateShotMakeProbability({ shotZone: 'rim', shooterProfile: profile, shooterFatigue: 0, opponentDefenseProxy: 50 })).toBeCloseTo(0.6465)
    expect(calculateShotMakeProbability({ shotZone: 'rim', shooterProfile: profile, shooterFatigue: 80, opponentDefenseProxy: 50 })).toBeCloseTo(0.5825)
    expect(calculateShotMakeProbability({ shotZone: 'rim', shooterProfile: profile, shooterFatigue: 0, opponentDefenseProxy: 80 })).toBeCloseTo(0.5865)
  })
})
