...
import { controlBallByPlayer, createMatchPlayerProfile, createMatchSession, createOffBallCutIntent, selectOffBallCutter, stepMatchSession, stepPlayersTowardBaseSpacing, substitutePlayer, updateOffBallCutIntent } from './index'
...
describe('off-ball cut movement', () => {
  it('selects only an active off-ball player and moves toward the cut target using their kinematics', () => {
    ...
    const selected = selectOffBallCutter(lineup, state.playerProfiles[offenseKey], handler, new FixedRandom([0, 0]))
    expect(selected).toBe(cutter)
    expect(selectOffBallCutter(lineup, state.playerProfiles[offenseKey], handler, new SeededRandomSource(99)))
      .toBe(selectOffBallCutter(lineup, state.playerProfiles[offenseKey], handler, new SeededRandomSource(99)))

    const basket = state.attackingTeamId === state.homeTeamId ? state.spatial.court.baskets.right : state.spatial.court.baskets.left
    const intent = createOffBallCutIntent({ teamId: state.attackingTeamId, playerId: cutter, spatial: state.spatial, attackingBasket: basket })
    const input = { homeTeamId: state.homeTeamId, awayTeamId: state.awayTeamId, attackingTeamId: state.attackingTeamId, period: state.period, activeLineups: state.activeLineups, playerProfiles: state.playerProfiles, spatial: state.spatial, ballHandlerId: handler }
    ...
    const cutterProfile = state.playerProfiles[offenseKey].find((profile) => profile.playerId === cutter)!
    const start = state.spatial.players.find((player) => player.playerId === cutter)!
    ...
    const fasterProfiles = replaceKinematics(state.playerProfiles, offenseKey, cutter, { ...cutterProfile.kinematics, maxSpeedMps: 6.6, accelerationMps2: 3.6 })
    const slowerProfiles = replaceKinematics(state.playerProfiles, offenseKey, cutter, { ...cutterProfile.kinematics, maxSpeedMps: 5.4, accelerationMps2: 2.4 })
