...
describe('PlayerMatchStats', () => {
  ...
  it('keeps partial minutes anti-spoiler and gives a productive regulation match 12000 player-seconds per team', () => {
    ...
    const elapsedSeconds = simulation.events.filter((event) => event.type === 'periodEnd').reduce((sum, event) => sum + (event.period <= 4 ? 600 : 300), 0)
    expect(stats.filter((stat) => homeIds.has(stat.playerId)).reduce((sum, stat) => sum + stat.secondsPlayed, 0)).toBe(elapsedSeconds * 5)
    expect(stats.filter((stat) => awayIds.has(stat.playerId)).reduce((sum, stat) => sum + stat.secondsPlayed, 0)).toBe(elapsedSeconds * 5)
