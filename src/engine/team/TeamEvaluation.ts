...
export function selectStartingFive(world: GameWorld, teamId: TeamId): readonly PlayerId[] { const roster=getTeamRoster(world,teamId); if(roster.length<5) throw new Error(`Team ${teamId} cannot form a starting five with only ${roster.length} players`); const selected:PlayerId[]=[]; for(const position of POSITIONS){const candidates=roster.filter(p=>p.basketball.primaryPosition===position&&!selected.includes(p.id));const pool=candidates.length?candidates:roster.filter(p=>!selected.includes(p.id));const chosen=[...pool].sort((a,b)=>calculatePlayerImpact(b)-calculatePlayerImpact(a)||(a.id<b.id?-1:1))[0];if(chosen)selected.push(chosen.id)}return selected }
export function calculateTeamStrength(world:GameWorld,teamId:TeamId):TeamStrength {const starters=selectStartingFive(world,teamId);const value=starters.reduce((sum,id)=>sum+calculatePlayerImpact(world.players[id]!),0)/5;return{teamId,value}}
