import { BASKETBALL_RATING_KEYS, calculateAge, calculateBootstrapAbilityProxy, createPlayer, type BasketballRatingKey, type Player } from '@/domain/player'
...
export function calculatePotentialGrowthFactor(player: Player): number {
  ...
  const remainingPotential = Math.max(0, player.potential.ceiling - currentAbilityProxy)
