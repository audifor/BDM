...
function worldFor(positions: readonly Stance[], access = 100) {
  ...
  const upgradedAssignments = [...Object.values(base.teamStaffAssignmentsById).map((assignment) => assignment.teamId !== teamId || assignment.staffPersonId === subjectId || assignment.role === 'headCoach' ? assignment : { ...assignment, role: 'generalManager' as never }), ...extras.map((extra) => extra.assignment)]
