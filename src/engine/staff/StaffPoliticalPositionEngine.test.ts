...
function fixture(options: { readonly relationship?: 'actorToSubject' | 'subjectToActor' | 'none'; readonly dimensions?: Record<string, number>; readonly caseKind?: 'RELEASE' | 'MORE_RESPONSIBILITY' | 'PROMOTION'; readonly holder?: boolean; readonly conflict?: boolean; readonly positions?: readonly { actorId: ReturnType<typeof staffPersonIdFromString>; stance: 'SUPPORT' | 'OPPOSE' | 'MEDIATE'; since: GameDate; lastEvaluatedOn: GameDate }[] } = {}) {
  ...
  const assignments = [subjectId, actorId].map((staffPersonId, index) => createTeamStaffAssignment({ id: teamStaffAssignmentIdFromString(`politics-assignment-${index}`), staffPersonId, teamId, role: index === 0 ? 'assistantCoach' : 'regionalScout', assignedOn: date }))
