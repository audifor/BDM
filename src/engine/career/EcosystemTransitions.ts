...
function requireRoute(world: GameWorld, playerId: PlayerId, destinationTeamId: TeamId, originKind: 'ncaaLike' | 'fibaLike' | 'nbaLike', destinationKind: 'fibaLike' | 'nbaLike') { const source = findRosterTeam(world, playerId); if (source === undefined) throw new Error('Player is not on an active roster'); const route = routeForTeams(world, source.id, destinationTeamId); if (world.ecosystems[route.fromEcosystemId]?.kind !== originKind || world.ecosystems[route.toEcosystemId]?.kind !== destinationKind) throw new Error('Invalid ecosystem transition route'); return { ...route, sourceTeamId: source.id } }
