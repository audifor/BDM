...
export function createStaffConflict(input: StaffConflict): StaffConflict {
  ...
  const startedOn = parseGameDate(input.startedOn); const lastEvaluatedOn = parseGameDate(input.lastEvaluatedOn)
  ...
  if (lastEvaluatedOn < startedOn) throw new RangeError('Staff conflict cannot be evaluated before it starts')
  if (participants.some((participant) => participant.leftOn !== undefined && participant.leftOn < participant.joinedOn)) throw new RangeError('Staff conflict participant cannot leave before joining')
  if (input.resolvedOn !== undefined && parseGameDate(input.resolvedOn) < startedOn) throw new RangeError('Staff conflict cannot resolve before it starts')
  if (input.resolution !== undefined && input.resolvedOn !== undefined && parseGameDate(input.resolution.resolvedOn) !== parseGameDate(input.resolvedOn)) throw new RangeError('Staff conflict resolution date must match resolved date')
