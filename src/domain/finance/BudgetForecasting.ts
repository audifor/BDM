...
export function getBudgetComparison(world: GameWorld, budgetId: string, asOfDate: GameDate | string = world.currentDate): readonly BudgetComparisonRow[] {
  ...
  return Object.freeze(getBudgetLines(world, budgetId).map((line) => {
    ...
    const forecast = actual + committed.total + assumptions
...
export function createFinancialForecast(world: GameWorld, input: { readonly organizationId: string; readonly asOfDate: GameDate | string; readonly period: FinancialPlanningPeriod; readonly currencyCode: string; readonly scenario?: ForecastScenario; readonly linkedBudgetId?: string | null }): FinancialForecast {
  ...
  const lines = [...categories.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([category, row]) => Object.freeze({ category, direction: row.direction, amount: createMoney({ currencyCode, minorUnits: row.actual + row.committed + row.assumption }), actualMinorUnits: row.actual, committedMinorUnits: row.committed, assumptionMinorUnits: row.assumption }))
...
function committedForLine(world: GameWorld, budget: FinancialBudget, line: BudgetLine, asOf: GameDate): { total: number; future: number } { if (line.direction !== 'EXPENSE') return { total: 0, future: 0 }; const entries = knownCommitments(world, budget.organizationId, budget.period, asOf, budget.currencyCode).filter((item) => item.category === line.category && (line.teamId === undefined || item.teamId === line.teamId) && (line.organizationSectionId === undefined || item.organizationSectionId === line.organizationSectionId)); return { total: entries.reduce((sum, item) => sum + item.amount, 0), future: entries.reduce((sum, item) => sum + (item.future ? item.amount : 0), 0) } }
