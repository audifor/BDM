...
export type SportsEcosystemKind = 'fibaLike' | 'nbaLike'
export interface SportsEcosystem { readonly id: EcosystemId; readonly name: string; readonly kind: SportsEcosystemKind; readonly domesticTiers: readonly DomesticCompetitionTier[]; readonly tierMovementRules: readonly TierMovementRule[] }
export function createSportsEcosystem(input: Omit<SportsEcosystem, 'domesticTiers' | 'tierMovementRules'> & Partial<Pick<SportsEcosystem, 'domesticTiers' | 'tierMovementRules'>>): SportsEcosystem {
  if (input.kind !== 'fibaLike' && input.kind !== 'nbaLike') throw new RangeError('Sports ecosystem kind is unsupported')
  ...
  if (input.kind === 'nbaLike' && (tiers.length !== 0 || rules.length !== 0)) throw new RangeError('NBA-like ecosystems cannot have domestic tier movement')
