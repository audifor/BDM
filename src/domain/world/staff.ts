import { calculateStaffRoleProficiency, type StaffRole } from '@/domain/staff'
...
export function getTeamStaffByRole(world:GameWorld,teamId:TeamId,role:StaffRole){return getTeamStaffAssignments(world,teamId).filter(a=>a.role===role).map(a=>world.staffPeopleById[a.staffPersonId]!)}
...
export function getStaffRoleProficiency(world:GameWorld,staffPersonId:StaffPersonId){const assignment=getStaffAssignment(world,staffPersonId);return assignment===undefined?undefined:calculateStaffRoleProficiency(world.staffPeopleById[staffPersonId]!,assignment.role)}
