...
export function getPendingMediaOpportunities(world: GameWorld, coachId: string): readonly MediaOpportunity[] { return Object.values(world.mediaOpportunitiesById).filter((item) => item.coachId === coachId && item.status === 'pending').sort((a, b) => b.importance - a.importance || b.gameDate.localeCompare(a.gameDate) || a.id.localeCompare(b.id)) }
export function getCoachMediaProfileDescriptor(world: GameWorld, coachId: string) { const profile = world.mediaProfilesByCoachId[coachId]; return profile === undefined ? undefined : getMediaProfileDescriptor(profile) }
