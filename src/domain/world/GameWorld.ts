...
function validateGovernance(world: GameWorld): void {
  ...
  for (const request of Object.values(world.governanceRequestsById)) {
    ...
    for (const event of events) { validateParty(world, request, event.actor, event.effectiveOn); const expected = event.kind === 'ISSUED' || event.kind === 'WITHDRAWN' ? request.issuer : event.kind === 'FULFILLED' ? undefined : request.recipient; if (expected !== undefined && !sameGovernanceInteractionParty(event.actor, expected)) throw new GameWorldValidationError(`Governance request ${request.id} event actor is invalid`); if (event.kind === 'FULFILLED' && event.evidence !== undefined && requireEntity(world.governanceDecisionsById, event.evidence.decisionId, 'Request fulfillment decision').institutionId !== request.institutionId) throw new GameWorldValidationError(`Governance request ${request.id} fulfillment decision crosses institution`) }
  }
