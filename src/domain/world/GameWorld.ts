...
function validateParty(world:GameWorld,meeting:GovernanceMeeting,p:GovernanceInteractionParty,on:GameDate):void{if(p.kind==='BODY'){if(requireEntity(world.governanceBodiesById,p.bodyId,'Meeting participant body').institutionId!==meeting.institutionId)throw new GameWorldValidationError('Meeting participant body crosses institution');return}if(p.kind==='APPOINTMENT'){const a=requireEntity(world.governanceAppointmentsById,p.appointmentId,'Meeting participant appointment');if(requireEntity(world.governanceBodiesById,a.bodyId,'Meeting participant appointment body').institutionId!==meeting.institutionId||a.startedOn>on||(a.endedOn!==undefined&&a.endedOn<on))throw new GameWorldValidationError('Meeting participant appointment inactive');return}if(p.actor.kind==='COACH')requireEntity(world.coaches,p.actor.id as CoachId,'Meeting participant coach');else requireEntity(world.staffPeopleById,p.actor.id as StaffPersonId,'Meeting participant staff')}
