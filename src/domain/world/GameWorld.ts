...
export interface GameWorld {
  ...
  readonly facilityControlRightsById: Readonly<Record<FacilityControlRightId, FacilityControlRight>>
  ...
  readonly governanceExternalRelationshipsById: Readonly<Record<string, GovernanceExternalRelationship>>
  readonly governanceExpectationPeriodsById: Readonly<Record<string, GovernanceExpectationPeriod>>
  readonly governanceObjectivesById: Readonly<Record<string, GovernanceObjective>>
  ...
  readonly governanceMeetingsById: Readonly<Record<string, GovernanceMeeting>>
  readonly governanceMeetingParticipantsById: Readonly<Record<string, GovernanceMeetingParticipant>>
  readonly governanceMeetingAgendaItemsById: Readonly<Record<string, GovernanceMeetingAgendaItem>>
  readonly governanceMeetingEventsById: Readonly<Record<string, GovernanceMeetingEvent>>
  readonly governanceRequestsById: Readonly<Record<string, GovernanceRequest>>
  readonly governanceRequestEventsById: Readonly<Record<string, GovernanceRequestEvent>>
  readonly governanceCommitmentsById: Readonly<Record<string, GovernanceCommitment>>
  readonly governanceCommitmentEventsById: Readonly<Record<string, GovernanceCommitmentEvent>>
...
export interface CreateGameWorldInput {
  ...
  facilityControlRights?: readonly FacilityControlRight[]
  ...
  governanceExternalRelationships?: readonly GovernanceExternalRelationship[]
  governanceExpectationPeriods?: readonly GovernanceExpectationPeriod[]
  governanceObjectives?: readonly GovernanceObjective[]
  ...
  governanceMeetings?: readonly GovernanceMeeting[]
  governanceMeetingParticipants?: readonly GovernanceMeetingParticipant[]
  governanceMeetingAgendaItems?: readonly GovernanceMeetingAgendaItem[]
  governanceMeetingEvents?: readonly GovernanceMeetingEvent[]
  governanceRequests?: readonly GovernanceRequest[]
  governanceRequestEvents?: readonly GovernanceRequestEvent[]
  governanceCommitments?: readonly GovernanceCommitment[]
  governanceCommitmentEvents?: readonly GovernanceCommitmentEvent[]
...
export function createGameWorld(input: CreateGameWorldInput): GameWorld {
  ...
  const world: GameWorld = {
    ...
    facilityControlRightsById: indexById((input.facilityControlRights ?? []).map(createFacilityControlRight), 'Facility control right'),
    ...
    governanceExternalRelationshipsById: indexById((input.governanceExternalRelationships ?? []).map(createGovernanceExternalRelationship), 'Governance external relationship'),
    governanceExpectationPeriodsById: indexById((input.governanceExpectationPeriods ?? []).map(createGovernanceExpectationPeriod), 'Governance expectation period'),
    governanceObjectivesById: indexById((input.governanceObjectives ?? []).map(createGovernanceObjective), 'Governance objective'),
    ...
    governanceMeetingsById: indexById((input.governanceMeetings ?? []).map(createGovernanceMeeting), 'Governance meeting'), governanceMeetingParticipantsById: indexById((input.governanceMeetingParticipants ?? []).map(createGovernanceMeetingParticipant), 'Governance meeting participant'), governanceMeetingAgendaItemsById: indexById((input.governanceMeetingAgendaItems ?? []).map(createGovernanceMeetingAgendaItem), 'Governance meeting agenda item'), governanceMeetingEventsById: indexById((input.governanceMeetingEvents ?? []).map(createGovernanceMeetingEvent), 'Governance meeting event'),
    governanceRequestsById: indexById((input.governanceRequests ?? []).map(createGovernanceRequest), 'Governance request'), governanceRequestEventsById: indexById((input.governanceRequestEvents ?? []).map(createGovernanceRequestEvent), 'Governance request event'),
    governanceCommitmentsById: indexById((input.governanceCommitments ?? []).map(createGovernanceCommitment), 'Governance commitment'), governanceCommitmentEventsById: indexById((input.governanceCommitmentEvents ?? []).map(createGovernanceCommitmentEvent), 'Governance commitment event'),
...
export function updateGameWorld(world: GameWorld, patch: Partial<CreateGameWorldInput>): GameWorld {
  ...
  if (inputKey === 'treasuryApplications') assertTreasurySettlementsAppendOnly(world, value as readonly TreasurySettlement[])
    ...
    if (inputKey === 'financialRegulationAssessments') assertImmutableCollection(world.financialRegulationAssessmentsById, (value as readonly FinancialRegulationAssessment[]).map(createFinancialRegulationAssessment), 'Financial assessment')
    if (inputKey === 'economicObservations') assertImmutableCollection(world.economicObservationsById, (value as readonly EconomicObservation[]).map(createEconomicObservation), 'Economic observation')
    if (inputKey === 'exchangeRates') assertImmutableCollection(world.exchangeRatesById, (value as readonly ExchangeRate[]).map(createExchangeRate), 'Exchange rate')
...
export function addMemoriesToGameWorld(world: GameWorld, additions: readonly MemoryRecord[]): GameWorld {
  const memoriesById: Record<string, MemoryRecord> = { ...world.memoriesById }
  let changed = false
  for (const input of additions) {
    const memory = createMemory(input)
    if (memoriesById[memory.id] !== undefined) continue
...
const collectionPatchIndexers: Readonly<Record<string, (value: unknown) => unknown>> = {
  ...
  financialRegulationAssessments: (value) => indexById((value as readonly FinancialRegulationAssessment[]).map(createFinancialRegulationAssessment), 'Financial regulation assessment'),
  economicObservations: (value) => indexById((value as readonly EconomicObservation[]).map(createEconomicObservation), 'Economic observation'),
  exchangeRates: (value) => indexById((value as readonly ExchangeRate[]).map(createExchangeRate), 'Exchange rate'),
  ...
  facilityControlRights: (value) => indexById((value as readonly FacilityControlRight[]).map(createFacilityControlRight), 'Facility control right'),
  ...
  facilityDevelopmentProjects: (value) => indexById((value as readonly FacilityDevelopmentProject[]).map(createFacilityDevelopmentProject), 'Facility development project'),
  facilityDevelopmentProjectPhases: (value) => indexById((value as readonly FacilityDevelopmentProjectPhase[]).map(createFacilityDevelopmentProjectPhase), 'Facility development project phase'),
  ...
  governanceExternalRelationships: (value) => indexById((value as readonly GovernanceExternalRelationship[]).map(createGovernanceExternalRelationship), 'Governance external relationship'),
  governanceExpectationPeriods: (value) => indexById((value as readonly GovernanceExpectationPeriod[]).map(createGovernanceExpectationPeriod), 'Governance expectation period'),
  governanceObjectives: (value) => indexById((value as readonly GovernanceObjective[]).map(createGovernanceObjective), 'Governance objective'),
...
function validateOrganizationOwnershipAndControl(world: GameWorld): void {
  for (const ownership of Object.values(world.organizationOwnershipById)) {
    createOrganizationOwnership(ownership)
    requireEntity(world.organizationsById, ownership.organizationId, `Organization ownership ${ownership.id} target`)
    if (ownership.owner.kind === 'PERSON') {
      requireEntity(world.personsById, ownership.owner.personId, `Organization ownership ${ownership.id} Person owner`)
    } else {
      requireEntity(world.organizationsById, ownership.owner.organizationId, `Organization ownership ${ownership.id} Organization owner`)
      if (ownership.owner.organizationId === ownership.organizationId) throw new GameWorldValidationError(`Organization ownership ${ownership.id} cannot self-own`)
    }
  }
  for (const control of Object.values(world.organizationControlById)) {
    createOrganizationControl(control)
    requireEntity(world.organizationsById, control.organizationId, `Organization control ${control.id} target`)
    if (control.controller.kind === 'PERSON') {
      requireEntity(world.personsById, control.controller.personId, `Organization control ${control.id} Person controller`)
    } else {
      requireEntity(world.organizationsById, control.controller.organizationId, `Organization control ${control.id} Organization controller`)
...
function validateFacilities(world: GameWorld): void {
  try {
    validateFacilitiesDomain({
      ...
      controlRights: Object.values(world.facilityControlRightsById),
      operatorAssignments: Object.values(world.facilityOperatorAssignmentsById),
      ...
      developmentProjects: Object.values(world.facilityDevelopmentProjectsById),
      developmentProjectPhases: Object.values(world.facilityDevelopmentProjectPhasesById),
...
function validateGovernance(world: GameWorld): void {
  ...
  for (const institution of Object.values(institutions)) {
    if (institution.parentInstitutionId !== undefined) requireEntity(institutions, institution.parentInstitutionId, `Governance institution ${institution.id} parent`)
    for (const teamId of institution.teamIds) {
      requireEntity(world.teams, teamId, `Governance institution ${institution.id} team`)
      if (linkedTeamIds.has(teamId)) throw new GameWorldValidationError(`Governance team ${teamId} linked more than once`)
      linkedTeamIds.add(teamId)
    }
    const seen = new Set<string>(); let current = institution
  ...
  for (const meeting of Object.values(world.governanceMeetingsById)) {
    ...
    const events=Object.values(world.governanceMeetingEventsById).filter(e=>e.meetingId===meeting.id); if(events.some(e=>e.effectiveOn<meeting.createdOn))throw new GameWorldValidationError(`Governance meeting ${meeting.id} event precedes creation`); try{validateGovernanceMeetingLifecycle(events)}catch(error){throw new GameWorldValidationError((error as Error).message)}; const heldOn=sortGovernanceMeetingEvents(events).find(e=>e.kind==='HELD')?.effectiveOn
    const keys=new Set<string>(); let chairs=0; for(const p of Object.values(world.governanceMeetingParticipantsById).filter(p=>p.meetingId===meeting.id)){if(heldOn===undefined)throw new GameWorldValidationError(`Governance meeting ${meeting.id} participants require held meeting`); const key=partyKey(p.party)+p.role;if(keys.has(key))throw new GameWorldValidationError(`Governance meeting ${meeting.id} duplicate participant`);keys.add(key);if(p.role==='CHAIR'&&p.attendance==='PRESENT'&&++chairs>1)throw new GameWorldValidationError(`Governance meeting ${meeting.id} multiple present chairs`);validateParty(world,meeting,p.party,heldOn)}
    const sequences=new Set<number>();for(const item of Object.values(world.governanceMeetingAgendaItemsById).filter(i=>i.meetingId===meeting.id)){if(sequences.has(item.sequence))throw new GameWorldValidationError(`Governance meeting ${meeting.id} duplicate agenda sequence`);sequences.add(item.sequence);if(item.subject.kind==='GOVERNANCE_DECISION'&&requireEntity(world.governanceDecisionsById,item.subject.decisionId,'Agenda decision').institutionId!==meeting.institutionId)throw new GameWorldValidationError('Agenda decision crosses institution');if(item.subject.kind==='GOVERNANCE_OBJECTIVE'){const o=requireEntity(world.governanceObjectivesById,item.subject.objectiveId,'Agenda objective');if(requireEntity(world.governanceExpectationPeriodsById,o.expectationPeriodId,'Agenda objective period').institutionId!==meeting.institutionId)throw new GameWorldValidationError('Agenda objective crosses institution')}if(item.subject.kind==='GOVERNANCE_MANAGER_EVALUATION'){const e=requireEntity(world.governanceManagerEvaluationsById,item.subject.evaluationId,'Agenda evaluation');if(requireEntity(world.governanceManagerEvaluationPeriodsById,e.evaluationPeriodId,'Agenda evaluation period').institutionId!==meeting.institutionId)throw new GameWorldValidationError('Agenda evaluation crosses institution')}if(item.subject.kind==='GOVERNANCE_REQUEST'&&requireEntity(world.governanceRequestsById,item.subject.requestId,'Agenda request').institutionId!==meeting.institutionId)throw new GameWorldValidationError('Agenda request crosses institution');if(item.subject.kind==='GOVERNANCE_COMMITMENT'&&requireEntity(world.governanceCommitmentsById,item.subject.commitmentId,'Agenda commitment').institutionId!==meeting.institutionId)throw new GameWorldValidationError('Agenda commitment crosses institution')}
  }
  for(const e of Object.values(world.governanceMeetingEventsById))requireEntity(world.governanceMeetingsById,e.meetingId,'Governance meeting event meeting');for(const p of Object.values(world.governanceMeetingParticipantsById))requireEntity(world.governanceMeetingsById,p.meetingId,'Governance meeting participant meeting');for(const i of Object.values(world.governanceMeetingAgendaItemsById))requireEntity(world.governanceMeetingsById,i.meetingId,'Governance meeting agenda meeting')
  for (const request of Object.values(world.governanceRequestsById)) {
    requireEntity(institutions, request.institutionId, `Governance request ${request.id} institution`)
    validateParty(world, request, request.issuer); validateParty(world, request, request.recipient)
    if (sameGovernanceInteractionParty(request.issuer, request.recipient)) throw new GameWorldValidationError(`Governance request ${request.id} issuer and recipient are the same`)
    if (request.origin.kind === 'MEETING') { const meeting = requireEntity(world.governanceMeetingsById, request.origin.meetingId, `Governance request ${request.id} origin meeting`); if (meeting.institutionId !== request.institutionId) throw new GameWorldValidationError(`Governance request ${request.id} origin meeting crosses institution`); if (request.origin.agendaItemId !== undefined) { const agenda = requireEntity(world.governanceMeetingAgendaItemsById, request.origin.agendaItemId, `Governance request ${request.id} origin agenda`); if (agenda.meetingId !== meeting.id) throw new GameWorldValidationError(`Governance request ${request.id} origin agenda belongs to another meeting`); if (agenda.subject.kind === 'GOVERNANCE_REQUEST' && agenda.subject.requestId === request.id) throw new GameWorldValidationError(`Governance request ${request.id} has circular agenda origin`) } }
    const events = Object.values(world.governanceRequestEventsById).filter((event) => event.requestId === request.id); try { validateGovernanceRequestLifecycle(events) } catch (error) { throw new GameWorldValidationError((error as Error).message) }; const issued = sortGovernanceRequestEvents(events)[0]!; if (request.dueOn !== undefined && request.dueOn < issued.effectiveOn) throw new GameWorldValidationError(`Governance request ${request.id} due date precedes issue`)
    for (const event of events) { validateParty(world, request, event.actor, event.effectiveOn); const expected = event.kind === 'ISSUED' || event.kind === 'WITHDRAWN' ? request.issuer : event.kind === 'FULFILLED' ? undefined : request.recipient; if (expected !== undefined && !sameGovernanceInteractionParty(event.actor, expected)) throw new GameWorldValidationError(`Governance request ${request.id} event actor is invalid`); if (event.kind === 'FULFILLED' && event.evidence !== undefined) { if (event.evidence.kind === 'GOVERNANCE_DECISION' && requireEntity(world.governanceDecisionsById, event.evidence.decisionId, 'Request fulfillment decision').institutionId !== request.institutionId) throw new GameWorldValidationError(`Governance request ${request.id} fulfillment decision crosses institution`); if (event.evidence.kind === 'GOVERNANCE_COMMITMENT') { const commitment = requireEntity(world.governanceCommitmentsById, event.evidence.commitmentId, 'Request fulfillment commitment'); if (commitment.institutionId !== request.institutionId) throw new GameWorldValidationError(`Governance request ${request.id} fulfillment commitment crosses institution`); const fulfilled = sortGovernanceCommitmentEvents(Object.values(world.governanceCommitmentEventsById).filter((item) => item.commitmentId === commitment.id)).find((item) => item.kind === 'FULFILLED'); if (fulfilled === undefined || fulfilled.effectiveOn > event.effectiveOn) throw new GameWorldValidationError(`Governance request ${request.id} fulfillment commitment is not fulfilled`) } } }
  }
  for (const event of Object.values(world.governanceRequestEventsById)) requireEntity(world.governanceRequestsById, event.requestId, 'Governance request event request')
  for (const commitment of Object.values(world.governanceCommitmentsById)) {
    requireEntity(institutions, commitment.institutionId, `Governance commitment ${commitment.id} institution`); validateParty(world, commitment, commitment.promisor); validateParty(world, commitment, commitment.beneficiary); if (sameGovernanceInteractionParty(commitment.promisor, commitment.beneficiary)) throw new GameWorldValidationError(`Governance commitment ${commitment.id} promisor and beneficiary are the same`)
    if (commitment.provenance.kind === 'MEETING') { const meeting = requireEntity(world.governanceMeetingsById, commitment.provenance.meetingId, `Governance commitment ${commitment.id} provenance meeting`); if (meeting.institutionId !== commitment.institutionId) throw new GameWorldValidationError(`Governance commitment ${commitment.id} provenance meeting crosses institution`); if (commitment.provenance.agendaItemId !== undefined) { const agenda = requireEntity(world.governanceMeetingAgendaItemsById, commitment.provenance.agendaItemId, `Governance commitment ${commitment.id} provenance agenda`); if (agenda.meetingId !== meeting.id || (agenda.subject.kind === 'GOVERNANCE_COMMITMENT' && agenda.subject.commitmentId === commitment.id)) throw new GameWorldValidationError(`Governance commitment ${commitment.id} has invalid provenance agenda`) } } else if (commitment.provenance.kind === 'REQUEST' && requireEntity(world.governanceRequestsById, commitment.provenance.requestId, `Governance commitment ${commitment.id} provenance request`).institutionId !== commitment.institutionId) throw new GameWorldValidationError(`Governance commitment ${commitment.id} provenance request crosses institution`); else if (commitment.provenance.kind === 'GOVERNANCE_DECISION' && requireEntity(world.governanceDecisionsById, commitment.provenance.decisionId, `Governance commitment ${commitment.id} provenance decision`).institutionId !== commitment.institutionId) throw new GameWorldValidationError(`Governance commitment ${commitment.id} provenance decision crosses institution`)
    const events = Object.values(world.governanceCommitmentEventsById).filter((event) => event.commitmentId === commitment.id); try { validateGovernanceCommitmentLifecycle(events) } catch (error) { throw new GameWorldValidationError((error as Error).message) }; const made = sortGovernanceCommitmentEvents(events)[0]!; if (commitment.dueOn !== undefined && commitment.dueOn < made.effectiveOn) throw new GameWorldValidationError(`Governance commitment ${commitment.id} due date precedes made date`)
    for (const event of events) { if ('actor' in event) validateParty(world, commitment, event.actor, event.effectiveOn); if ((event.kind === 'MADE' || event.kind === 'WITHDRAWN') && !sameGovernanceInteractionParty(event.actor, commitment.promisor)) throw new GameWorldValidationError(`Governance commitment ${commitment.id} event actor is invalid`); if (event.kind === 'BREACHED' && (commitment.dueOn === undefined || event.effectiveOn <= commitment.dueOn)) throw new GameWorldValidationError(`Governance commitment ${commitment.id} breach is invalid`); if (event.kind === 'FULFILLED' && event.evidence !== undefined && requireEntity(world.governanceDecisionsById, event.evidence.decisionId, 'Commitment fulfillment decision').institutionId !== commitment.institutionId) throw new GameWorldValidationError(`Governance commitment ${commitment.id} fulfillment decision crosses institution`); if (event.kind === 'SUPERSEDED') { const successor = requireEntity(world.governanceCommitmentsById, event.successorCommitmentId, 'Commitment successor'); if (successor.id === commitment.id || successor.institutionId !== commitment.institutionId) throw new GameWorldValidationError(`Governance commitment ${commitment.id} successor is invalid`); const successorMade = sortGovernanceCommitmentEvents(Object.values(world.governanceCommitmentEventsById).filter((item) => item.commitmentId === successor.id))[0]; if (successorMade === undefined || successorMade.effectiveOn < made.effectiveOn) throw new GameWorldValidationError(`Governance commitment ${commitment.id} successor chronology is invalid`) } }
  }
  for (const event of Object.values(world.governanceCommitmentEventsById)) requireEntity(world.governanceCommitmentsById, event.commitmentId, 'Governance commitment event commitment')
  for (const commitment of Object.values(world.governanceCommitmentsById)) { const seen = new Set<string>(); let current = commitment; while (true) { if (seen.has(current.id)) throw new GameWorldValidationError(`Governance commitment supersession cycle at ${commitment.id}`); seen.add(current.id); const supersession = Object.values(world.governanceCommitmentEventsById).find((event): event is Extract<GovernanceCommitmentEvent, { kind: 'SUPERSEDED' }> => event.commitmentId === current.id && event.kind === 'SUPERSEDED'); if (supersession === undefined) break; current = requireEntity(world.governanceCommitmentsById, supersession.successorCommitmentId, 'Commitment successor') } }
  const grants = Object.values(world.governanceAuthorityGrantsById)
  for (const grant of grants) { requireEntity(world.governanceBodiesById, grant.fromBodyId, `Governance authority ${grant.id} source`); requireEntity(world.governanceBodiesById, grant.toBodyId, `Governance authority ${grant.id} target`) }
  for (const relationship of Object.values(world.governanceExternalRelationshipsById)) {
    if (world.governanceBodiesById[relationship.id] !== undefined) throw new GameWorldValidationError(`Governance external relationship ID collides with body ${relationship.id}`)
    if (relationship.institutionId !== undefined) requireEntity(institutions, relationship.institutionId, `Governance external relationship ${relationship.id} institution`)
    if (relationship.bodyId !== undefined) {
      const body = requireEntity(world.governanceBodiesById, relationship.bodyId, `Governance external relationship ${relationship.id} body`)
      if (relationship.institutionId !== undefined && body.institutionId !== relationship.institutionId) throw new GameWorldValidationError(`Governance external relationship ${relationship.id} body belongs to another institution`)
    }
    if (relationship.externalRef.kind === 'CONFERENCE') requireEntity(world.conferencesById, relationship.externalRef.id, `Governance external relationship ${relationship.id} conference`)
  }
  for (const period of Object.values(world.governanceExpectationPeriodsById)) { const institution = requireEntity(institutions, period.institutionId, `Governance expectation period ${period.id} institution`); if (period.universe !== institution.universe) throw new GameWorldValidationError(`Governance expectation period ${period.id} universe differs from its institution`) }
  for (const objective of Object.values(world.governanceObjectivesById)) {
    const period = requireEntity(world.governanceExpectationPeriodsById, objective.expectationPeriodId, `Governance objective ${objective.id} period`)
    if (objective.ownerInstitutionId !== undefined) requireEntity(institutions, objective.ownerInstitutionId, `Governance objective ${objective.id} institution`)
    if (objective.ownerBodyId !== undefined) {
      const body = requireEntity(world.governanceBodiesById, objective.ownerBodyId, `Governance objective ${objective.id} body`)
      if (objective.ownerInstitutionId !== undefined && body.institutionId !== objective.ownerInstitutionId) throw new GameWorldValidationError(`Governance objective ${objective.id} owner body belongs to another institution`)
      if (body.institutionId !== period.institutionId) throw new GameWorldValidationError(`Governance objective ${objective.id} owner body differs from its period`)
    }
    if (objective.ownerInstitutionId !== undefined && objective.ownerInstitutionId !== period.institutionId) throw new GameWorldValidationError(`Governance objective ${objective.id} owner institution differs from its period`)
    if (objective.evaluationStartsOn < period.startedOn || (period.endedOn !== undefined && objective.evaluationEndsOn > period.endedOn)) throw new GameWorldValidationError(`Governance objective ${objective.id} evaluation window must be within its period`)
  }
  for (const period of Object.values(world.governanceManagerEvaluationPeriodsById)) {
    const institution = requireEntity(institutions, period.institutionId, `Governance manager evaluation period ${period.id} institution`)
    if (institution.universe !== period.universe) throw new GameWorldValidationError(`Governance manager evaluation period ${period.id} universe differs from its institution`)
    if (period.manager.kind === 'COACH') requireEntity(world.coaches, period.manager.id as CoachId, `Governance manager evaluation period ${period.id} coach`)
    else requireEntity(world.staffPeopleById, period.manager.id as StaffPersonId, `Governance manager evaluation period ${period.id} staff`)
  }
  for (const evaluation of Object.values(world.governanceManagerEvaluationsById)) {
    const period = requireEntity(world.governanceManagerEvaluationPeriodsById, evaluation.evaluationPeriodId, `Governance manager evaluation ${evaluation.id} period`)
    const body = requireEntity(world.governanceBodiesById, evaluation.evaluatorBodyId, `Governance manager evaluation ${evaluation.id} body`)
    if (body.institutionId !== period.institutionId || evaluation.evaluatedOn < period.startedOn || (period.endedOn !== undefined && evaluation.evaluatedOn > period.endedOn)) throw new GameWorldValidationError(`Governance manager evaluation ${evaluation.id} is outside its institutional period`)
    for (const objectiveEvaluation of evaluation.objectiveEvaluations) requireEntity(world.governanceObjectivesById, objectiveEvaluation.objectiveId, `Governance manager evaluation ${evaluation.id} objective`)
    for (const factor of evaluation.factors) {
      const source = factor.source
      if (source.kind === 'BG2_OBJECTIVE_EVALUATION') requireEntity(world.governanceObjectivesById, source.objectiveId, `Governance manager evaluation ${evaluation.id} factor objective`)
      if (source.kind === 'BG2_OBJECTIVE_SET_EVALUATION') for (const objectiveId of source.objectiveIds) requireEntity(world.governanceObjectivesById, objectiveId, `Governance manager evaluation ${evaluation.id} factor objective`)
      if (source.kind === 'GOVERNANCE_BODY') requireEntity(world.governanceBodiesById, source.bodyId, `Governance manager evaluation ${evaluation.id} factor body`)
      if (source.kind === 'STAFF_EMPLOYMENT') requireEntity(world.staffPeopleById, source.staffId as StaffPersonId, `Governance manager evaluation ${evaluation.id} factor employment staff`)
      if (source.kind === 'STAFF_CONTRACT') { requireEntity(world.staffPeopleById, source.staffId as StaffPersonId, `Governance manager evaluation ${evaluation.id} factor contract staff`); requireEntity(world.staffContractsById, source.contractId as never, `Governance manager evaluation ${evaluation.id} factor contract`) }
      if (source.kind === 'PRIOR_MANAGER_EVALUATION') requireEntity(world.governanceManagerEvaluationsById, source.evaluationId, `Governance manager evaluation ${evaluation.id} prior evaluation`)
      if (source.kind === 'STAFF_POLITICS') { requireEntity(world.teams, source.teamId as TeamId, `Governance manager evaluation ${evaluation.id} politics team`); for (const id of source.politicalCaseIds) requireEntity(world.staffPoliticalCasesById, id, `Governance manager evaluation ${evaluation.id} political case`); for (const id of source.politicalActionIds) requireEntity(world.staffPoliticalActionsById, id, `Governance manager evaluation ${evaluation.id} political action`) }
    }
  }
  for (const transition of Object.values(world.governanceJobSecurityTransitionsById)) {
    const evaluation = requireEntity(world.governanceManagerEvaluationsById, transition.evaluationId, `Governance job-security transition ${transition.id} evaluation`)
    const period = requireEntity(world.governanceManagerEvaluationPeriodsById, evaluation.evaluationPeriodId, `Governance job-security transition ${transition.id} period`)
...
function synchronizePersonRoots(world: GameWorld): Readonly<Record<import('@/domain/ids').PersonId, Person>> {
  ...
  for (const person of previous.values()) {
    ...
    if (person.profileRefs.every((ref) => ref.kind !== 'player' && ref.kind !== 'staff')) merged.set(person.id, person)
  }
...
function validateFinancialState(world: GameWorld): void {
  validateEconomicCollections(Object.values(world.economicObservationsById), Object.values(world.exchangeRatesById))
  for (const assessment of Object.values(world.financialRegulationAssessmentsById)) {
    requireEntity(world.organizationsById, assessment.organizationId, `Financial assessment ${assessment.id} organization`)
    requireEntity(world.organizationsById, assessment.authorityOrganizationId, `Financial assessment ${assessment.id} authority`)
    if (assessment.competitionId !== null) requireEntity(world.competitions, assessment.competitionId as CompetitionId, `Financial assessment ${assessment.id} competition`)
  }
