...
afterEach(() => { cleanup(); useEntityActionComposerStore.getState().close() })
const renderComposer = () => { cleanup(); return render(createElement(EntityActionComposer, { onResult: () => ({ kind: 'noExecutor' }) })).container.innerHTML }
...
describe('EntityActionComposer', () => {
  ...
  it('renders exactly four screen-independent quick slots', () => {
    ...
    expect(markup).toContain('<strong>talk</strong>'); expect(markup).toContain('<strong>limit</strong>')
    useEntityActionComposerStore.getState().close()
  })
