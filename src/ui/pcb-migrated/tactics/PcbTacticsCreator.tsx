...
export default function TacticsCreator({ clubId, onSaveCustomPlay }: TacticsCreatorProps) {
  ...
  // --- SAFE GUARD (SOLUCIÃ“N AL CRASH AL BORRAR) ---
  // Calculamos un Ã­ndice seguro. Si currentFrameIndex apunta fuera del array (porque borramos el Ãºltimo), usamos el Ãºltimo disponible.
  ...
  const generateEngineData = () => {
    // Usar la funciÃ³n de anÃ¡lisis del playbookManager para datos mÃ¡s completos
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (player) {
        ...
        if (activeTool === 'move' && player.id === currentFrame.ballOwnerId) { setWarning(`âš ï¸ Â¡PASOS! ${player.label} tiene balÃ³n.`); setTimeout(() => setWarning(null), 3000); }
    }
  };
  ...
  const deleteFrame = (index: number) => {
    ...
    setFrames(newFrames);
    // Ajustar Ã­ndice si es necesario para evitar apuntar fuera
