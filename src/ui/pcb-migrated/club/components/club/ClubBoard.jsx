...
export default function ClubBoard({
  ...
  const [catalog, setCatalog] = useState(MIGRATION_OBJECTIVE_CATALOG);
  ...
  const getConfidenceLevel = (conf) => {
    ...
    return { level: "critical", label: "CrÃ­tica", color: "#ef4444" };
  };

  const confidenceData = getConfidenceLevel(confidence);

  // Obtener definiciÃ³n de objetivo del catÃ¡logo
  ...
  const calculateProgress = (objective) => {
    ...
    // Verificar si estÃ¡ en peligro (season > mitad y progreso < 30%)
  ...
  const ObjectiveCard = ({ objective }) => {
    ...
    const typeConfig = {
      primary: { label: "Objetivo Principal", color: "#3b82f6", icon: "ðŸŽ¯" },
      secondary: { label: "Objetivo Secundario", color: "#8b5cf6", icon: "ðŸ“Œ" },
      financial: { label: "Objetivo Financiero", color: "#10b981", icon: "ðŸ’°" },
    };
            ...
            <ul className="consequences-list">
              {def.penalties.map((penalty, idx) => (
                <li key={idx}>
                  ...
                  {penalty.dismissal && (
                    <span className="dismissal-warning">âš ï¸ Posible despido</span>
                  )}
                </li>
              ))}
            </ul>
    ...
    return (
      <div className="card hero modal-glass-tactical confidence-panel">
        ...
        <div className="confidence-effects">
          ...
          <div className="effect-item">
            <span className="effect-label">Libertad TÃ¡ctica:</span>
            <span className="effect-value">
        ...
        {confidence < 55 && (
          <div className="confidence-warning">
            ...
            <span>
              {confidence < 40
                ? "Â¡AtenciÃ³n! Tu posiciÃ³n estÃ¡ en peligro. La directiva podrÃ­a despedirte."
                : "La directiva estÃ¡ preocupada. Debes mejorar los resultados pronto."}
            </span>
          </div>
        )}
      </div>
  ...
  // Modal de negociaciÃ³n de objetivos
        ...
        >
          ...
          <div className="modal-body">
            ...
            <div className="negotiation-info">
              ...
              <div className="info-item">
                <span className="info-label">Probabilidad de Ã‰xito:</span>
    ...
    <section className="bento club-board">
      <div className="card hero modal-glass-tactical">
        <div className="card-header">
          ...
          <span className="pill">Objetivos & PresiÃ³n</span>
        </div>
        <div className="desc">
          La directiva establece objetivos cada temporada. Tu confianza determina el presupuesto
          disponible, libertad tÃ¡ctica y seguridad laboral.
        </div>
        <button
      ...
      <div className="objectives-sections">
        {primaryObjectives.length > 0 && (
          <div className="objectives-group">
            <h3 className="group-title">ðŸŽ¯ Objetivos Principales</h3>
        ...
        {secondaryObjectives.length > 0 && (
          <div className="objectives-group">
            <h3 className="group-title">ðŸ“Œ Objetivos Secundarios</h3>
            <div className="objectives-grid">
        ...
        {financialObjectives.length > 0 && (
          <div className="objectives-group">
            <h3 className="group-title">ðŸ’° Objetivos Financieros</h3>
            <div className="objectives-grid">
        ...
        {hasObjectives && !hasCatalog && !catalogLoaded && (
          <div className="empty-state">
            <p>Cargando catÃ¡logo de objetivos...</p>
          </div>
        )}

        {hasObjectives && !hasCatalog && catalogLoaded && (
          <div className="empty-state">
            <p>Sin catÃ¡logo de objetivos disponible.</p>
          </div>
        )}
        ...
        {!hasObjectives && (
          <div className="empty-state">
            ...
            <p className="hint">
              Los objetivos se generan automÃ¡ticamente al inicio de cada temporada segÃºn tu
              divisiÃ³n y recursos del club.
            </p>
          </div>
        )}
      </div>
