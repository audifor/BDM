...
import { listFreeAgentStaff } from '@/app/staffCareer'
...
export function ClubPcbPage({ initialTab = 'dashboard', onFireStaff, onNegotiateStaff, world }: { readonly initialTab?: ClubTab; readonly onFireStaff?: (staffId: StaffPersonId) => void; readonly onNegotiateStaff?: (roleId: StaffRoleId, staffId: StaffPersonId) => void; readonly world?: GameWorld }) {
  ...
  return <section className="pcb-club" aria-label="Club PCB migrado">
    ...
    {tab === 'staff' && (staffData === undefined ? <section className="content-panel">No hay mundo de juego activo.</section> : <ClubStaffAssignments marketCandidates={staffData.marketCandidates} onFireStaff={onFireStaff} onNegotiateStaff={onNegotiateStaff} roles={staffData.roles} staffMembers={staffData.staffMembers} />)}
...
function getCanonicalStaffData(world: GameWorld) {
  ...
  const marketCandidates = listFreeAgentStaff(world).map((staffId) => {
    const staff = world.staffPeopleById[staffId]!
    const marketRole = staff.marketRole!
    return { id: staff.id, name: `${staff.identity.firstName} ${staff.identity.lastName}`, marketRole, proficiency: calculateStaffRoleProficiencyByRoleId(staff, marketRole) }
  })
