...
export function ClubPcbPage({ initialTab = 'dashboard' }: { readonly initialTab?: ClubTab }) {
  ...
  const onAssignPlayerToCoach = (coachId: number | null, playerId: number) => setTeamPlayers((value) => value.map((player) => (player.id === playerId ? { ...player, assigned_coach: coachId } : player)))
