...
import {
  acceptStaffRecommendation,
  dismissStaffRecommendation,
} from "@/app/staffRecommendations";
...
import {
  ...
  STAFF_UNIT_COHESION_DIMENSION_LABELS,
...
function userTeamId(w: GameWorld): TeamId {
  ...
  if (team === undefined)
    throw new Error("Expected a user team in the fixture world");
  return team.id;
}
...
function staffWithRole(
  ...
  const assignment = getTeamStaffAssignments(w, teamId).find(
    (item) => item.role === role,
  );
  if (assignment === undefined)
    throw new Error(
      `Expected an assignment with role ${role} on team ${teamId}`,
    );
...
describe("StaffScreen", () => {
  ...
  it("defaults selection to the first staff row and opens the detail panel for the initially selected staff person", () => {
    ...
    render(
      <StaffScreen
        initialSelectedStaffId={second.staffPersonId}
        teamId={teamId}
        world={w}
      />,
    );
    ...
    expect(
      screen.getAllByText(STAFF_ROLE_LABELS[second.role]).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("does not crash for a team id with no assigned team", () => {
    ...
    expect(
      screen.getByText("No team assigned to the user coach."),
    ).toBeTruthy();
  });
  ...
  it("opens the RESPONSIBILITIES tab and renders the staff-responsibilities grid with real responsibilities", () => {
    ...
    expect(
      screen.getByRole("region", { name: "staff-responsibilities" }),
    ).toBeTruthy();
    expect(
      screen.getAllByText(RESPONSIBILITY_KIND_LABELS.createTeamTrainingPlan)
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it("shows CONTROL/HOLDER/LOAD columns in the responsibilities grid", () => {
    ...
    const delegated = setTeamResponsibility(w, {
      teamId,
      kind: "createTeamTrainingPlan",
      mode: "delegated",
      holderStaffId: assistantId,
    });
  ...
  it("does not render an unsupported mode as a selectable control in the inspector", () => {
    ...
    fireEvent.doubleClick(
      screen.getAllByText(RESPONSIBILITY_KIND_LABELS.oppositionScouting)[0]!,
    );
    ...
    const labels = [...modeGroup.querySelectorAll("button")].map(
  ...
  it("delegated/advisory shows eligible Staff selector; choosing a candidate and Apply calls the callback", () => {
    ...
    fireEvent.doubleClick(
      screen.getAllByText(
        RESPONSIBILITY_KIND_LABELS.createTeamTrainingPlan,
      )[0]!,
    );
    ...
    expect(onSetResponsibility).toHaveBeenCalledWith({
      teamId,
      kind: "createTeamTrainingPlan",
      mode: "delegated",
      holderStaffId: assistantId,
  ...
  it("changing the selected Responsibility resets the draft", () => {
    ...
    expect(
      screen
        .getByRole("button", { name: "USER CONTROLLED" })
  ...
  it("shows RESPONSIBILITIES HELD in Staff detail, listing only responsibilities held by that staff person", () => {
    ...
    render(
      <StaffScreen
        initialSelectedStaffId={assistantId}
        teamId={teamId}
        world={delegated}
      />,
    );
  ...
  it("treatmentRecommendation never offers DELEGATED as a control mode", () => {
    ...
    fireEvent.doubleClick(
      screen.getAllByText(
        RESPONSIBILITY_KIND_LABELS.treatmentRecommendation,
      )[0]!,
    );
    const modeGroup = document.querySelector(".staff-mode-group")!;
    const labels = [...modeGroup.querySelectorAll("button")].map(
  ...
  it("treatmentRecommendation + ADVISORY shows an eligible physiotherapist; selecting them and Apply calls the callback with mode advisory", () => {
    ...
    fireEvent.click(
      within(document.querySelector(".staff-mode-group")!).getByRole("button", {
        name: "ADVISORY",
      }),
    );
    const select = screen.getByRole("combobox") as HTMLSelectElement;
    ...
    expect(
      select.querySelector(`option[value="${physiotherapistId}"]`),
    ).toBeTruthy();
    ...
    expect(onSetResponsibility).toHaveBeenCalledWith({
      ...
      mode: "advisory",
      holderStaffId: physiotherapistId,
    });
  });
  ...
  it("a stale/invalid draft holder (no longer a candidate) keeps APPLY disabled", () => {
    ...
    const reassignedAssignments = Object.values(w.teamStaffAssignmentsById).map(
      (assignment) =>
        assignment.staffPersonId === physiotherapistId
          ? { ...assignment, role: "assistantCoach" }
          : assignment,
    );
    const staleWorld = updateGameWorld(w, {
      ...
      staffEmploymentByStaffId: {
        ...w.staffEmploymentByStaffId,
        [physiotherapistId]: {
          ...w.staffEmploymentByStaffId[physiotherapistId],
...
type AdvisoryStaffAttributes = Record<
  (typeof STAFF_PROFESSIONAL_ATTRIBUTE_KEYS)[number],
  number
>;
const advisoryFlatAttributes: AdvisoryStaffAttributes = Object.fromEntries(
  STAFF_PROFESSIONAL_ATTRIBUTE_KEYS.map((key) => [key, 60]),
) as AdvisoryStaffAttributes;

function withAdvisoryStaffInRole(
  world: GameWorld,
  teamId: TeamId,
  role: string,
  kind: "treatmentRecommendation" | "contractRecommendation",
) {
  const staffId = staffPersonIdFromString(
    `advisory-ui-staff-${role}-${kind}-${teamId}`,
  );
  const withStaff = updateGameWorld(world, {
    staffPeople: [
      ...
      {
        id: staffId,
        identity: { firstName: "Med", lastName: "Ic" },
        professional: { attributes: advisoryFlatAttributes },
      },
    ],
    teamStaffAssignments: [
      ...Object.values(world.teamStaffAssignmentsById),
      {
        id: teamStaffAssignmentIdFromString(
          `advisory-ui-assignment-${role}-${kind}-${teamId}`,
        ),
  ...
  const delegated = updateGameWorld(withStaff, {
    responsibilities: [
      ...Object.values(withStaff.responsibilitiesById).filter(
        (responsibility) => responsibility.id !== id,
      ),
      { id, teamId, kind, mode: "advisory", holderStaffId: staffId },
    ],
  });
...
function withAdvisoryActiveInjury(world: GameWorld, teamId: TeamId) {
  ...
  const injury = createInjury({
    id: injuryIdFromString(`advisory-ui-injury-${teamId}`),
    playerId,
    kind: "ankleSprain",
    severity: "moderate",
...
function medicalPendingWorld() {
  ...
  const { world: withStaff, staffId } = withAdvisoryStaffInRole(
    withInjury,
    teamId,
    "teamDoctor",
  ...
  const outcome = Object.values(progressed.delegationOutcomesById).find(
    (item) =>
      item.staffId === staffId && item.kind === "treatmentRecommendation",
  )!;
...
function contractInformationalWorld() {
  ...
  const withOutcome = updateGameWorld(withStaff, {
    delegationOutcomes: [
      ...Object.values(withStaff.delegationOutcomesById),
      {
        id: outcomeId,
        responsibilityId:
          `responsibility:${teamId}:contractRecommendation` as never,
        staffId,
        decidedOn: withStaff.currentDate,
        kind: "contractRecommendation",
        applied: false,
        qualityScore: 60,
        payload: {
          playerId: playerId ?? "",
          ...
          recommendedAnnualSalary: 1000000,
...
describe("StaffScreen Advisory tab", () => {
  ...
  it("ADVISORY tab exists and renders the staff-advisory grid, defaulting to OPEN", () => {
    ...
    const filterGroup = document.querySelector<HTMLElement>(
      ".staff-advisory-toolbar .staff-mode-group",
    )!;
    expect(
      within(filterGroup)
  ...
  it("HISTORY filter shows accepted/dismissed and hides open recommendations", () => {
    ...
    const accepted = updateGameWorld(world, {
      delegationOutcomes: [
        ...Object.values(world.delegationOutcomesById).filter(
        ...
        ),
        {
          ...
          userDecidedOn: world.currentDate,
        },
      ],
    });
    ...
    fireEvent.click(
      within(filterGroup).getByRole("button", { name: "HISTORY" }),
    );
    expect(
      screen.getByRole("region", { name: "staff-advisory" }).textContent,
    ).toContain("ACCEPTED");
  });
  ...
  it("DISMISS calls the callback with the correct outcomeId", () => {
    ...
    const onDismissRecommendation = vi
      .fn()
      .mockReturnValue({ ok: true, world });
    render(
      <StaffScreen
        onDismissRecommendation={onDismissRecommendation}
        teamId={teamId}
...
describe("StaffScreen Dynamics tab", () => {
  ...
  it("NEEDS ATTENTION filter narrows the grid to Staff with needsAttention signals", () => {
    ...
    fireEvent.click(
      within(filterGroup).getByRole("button", { name: "NEEDS ATTENTION" }),
    );
    expect(
      within(filterGroup)
  ...
  it("selecting a Staff row opens the inspector with real explainability content", () => {
    ...
    const targetRow = rows.find((row) =>
      row.textContent?.includes(w.staffPeopleById[staffId]!.identity.firstName),
    );
  ...
  it("Wave 5B: Working Relationships appears in the inspector after a real Responsibility grant produces an attributed relationship", () => {
    ...
    const section = screen
      .getByText("WORKING RELATIONSHIPS")
      .closest("section")!;
...
function subviewGroup(): HTMLElement {
  return document.querySelector<HTMLElement>(
    ".staff-dynamics-subview .staff-mode-group",
  )!;
}
...
describe("StaffScreen Dynamics UNITS subview", () => {
  ...
  it("switching to UNITS renders the staff-units grid with one row per resolved unit", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /^DYNAMICS/ }));
    fireEvent.click(
      within(subviewGroup()).getByRole("button", { name: "UNITS" }),
    );
    ...
    for (const unit of units)
      expect(screen.getAllByText(unit.departmentLabel).length).toBeGreaterThan(
        0,
      );
  });
  ...
  it("double-clicking a unit row opens its inspector with qualitative bands only — no raw cohesion numbers", () => {
    ...
    fireEvent.doubleClick(
      screen.getAllByText(target.departmentLabel)[0]!.closest("tr")!,
    );
    ...
    for (const dimension of STAFF_UNIT_COHESION_DIMENSIONS) {
      expect(inspector.textContent).toContain(
        STAFF_UNIT_COHESION_DIMENSION_LABELS[dimension],
      );
    }
  ...
  it("a unit with no cohesion state yet degrades gracefully rather than crashing", () => {
    ...
    expect(screen.getByRole("region", { name: "staff-units" })).toBeTruthy();
    expect(
      screen.getByText("Unit cohesion has not been established yet."),
    ).toBeTruthy();
  });
  ...
  it("opens the POLITICS workspace with an accurate empty active state and history filter", () => {
    const w = createNewGame();
    render(<StaffScreen teamId={userTeamId(w)} world={w} />);
    fireEvent.click(screen.getByRole("button", { name: /^POLITICS/ }));
    expect(screen.getByText("POLITICAL CASES")).toBeTruthy();
    expect(
      screen.getByText("No political cases recorded for this organization."),
    ).toBeTruthy();
