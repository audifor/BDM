...
export function StandingsScreen({ world }: StandingsScreenProps) {
  ...
  return <section className="screen">
    <div className="page-heading"><div><p className="eyebrow">STANDINGS {champion === undefined ? '' : '· FINAL'}</p><h1>{competition.name}</h1></div><select value={competition.id} onChange={(event) => setSelectedId(event.target.value as typeof competition.id)}>{competitions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
