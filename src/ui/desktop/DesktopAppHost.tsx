...
import type { CoachPerkId, CoachSkillId, PlayerId, StaffPersonId, TeamId } from '@/domain/ids'
...
export function DesktopAppHost({ appId, entityDestination, world, actions }: { readonly appId: string; readonly entityDestination?: EntityDestination; readonly world: GameWorld; readonly actions: DesktopAppActions }) {
  ...
  if (appId === 'mentoring') return <MentoringPcbPage world={world} />
...
export interface DesktopAppActions { readonly tacticalPlan: MatchTacticalPlan; readonly openApp: (id: string) => void; readonly openEntity?: (destination: EntityDestination) => void; readonly playGame: () => void; readonly instantResult: () => void; readonly simulateRemainingGamesToday: () => void; readonly advanceDay: () => void; readonly startNextSeason: () => void; readonly releasePlayer: (teamId: TeamId, playerId: PlayerId) => void; readonly signFreeAgent: (teamId: TeamId, playerId: PlayerId) => void; readonly startStaffCandidacy: (teamId: TeamId, roleId: StaffRoleId, staffId: StaffPersonId) => void; readonly startStaffInterview: (candidacyId: string) => void; readonly completeStaffInterview: (candidacyId: string) => void; readonly createStaffOffer: (candidacyId: string) => void; readonly acceptStaffOffer: (offerId: string) => void; readonly declineStaffOffer: (offerId: string) => void; readonly fireStaff: (staffId: StaffPersonId) => void; readonly setStaffResponsibility?: (input: SetTeamResponsibilityInput) => void; readonly acceptStaffRecommendation?: (outcomeId: DelegationOutcomeId) => StaffRecommendationCommandResult; readonly dismissStaffRecommendation?: (outcomeId: DelegationOutcomeId) => StaffRecommendationCommandResult; readonly selectDraftProspect: (draftId: string, playerId: PlayerId) => void; readonly executeTrade?: (proposal: import('@/domain/trade').TradeProposal) => void; readonly purchaseSkill: (id: CoachSkillId) => void; readonly purchasePerk: (id: CoachPerkId) => void; readonly acceptOffer: (id: string) => void; readonly declineOffer: (id: string) => void; readonly applyForJob: (id: string) => void; readonly setTacticalPlan: (plan: MatchTacticalPlan) => void; readonly resetTacticalPlan: () => void; readonly setTrainingIntensity: (value: TrainingIntensity) => void; readonly setTrainingFocus: (value: TrainingFocus) => void; readonly scheduleTrainingSession: (session: ScheduledTrainingSession) => void; readonly scheduleTeamModuleSession: (input: { readonly moduleId: string; readonly date: GameWorld['currentDate']; readonly startTime: string; readonly durationMinutes: number; readonly sessionId: string; readonly intensity?: TrainingIntensity; readonly assignedStaffPersonIds?: readonly StaffPersonId[] }) => void; readonly cancelTrainingSession: (sessionId: string) => void; readonly saveUserTrainingModule: (module: UserTrainingModule) => void; readonly deleteUserTrainingModule: (moduleId: string) => void; readonly assignTrainingModuleToPlayer: (input: { readonly playerId: PlayerId; readonly moduleId: string; readonly date: GameWorld['currentDate']; readonly startTime: string; readonly sessionId: string; readonly assignedStaffPersonIds?: readonly StaffPersonId[] }) => void; readonly setLineupSlot: (slot: import('@/domain/tactics').LineupSlot, playerId: PlayerId) => void; readonly clearLineupSlot: (slot: import('@/domain/tactics').LineupSlot) => void; readonly updateRotationMinutes: (minutesByPeriod: Readonly<Record<PlayerId, readonly number[]>>) => void; readonly updateGamePlanMatchups: (matchups: readonly import('@/domain/tactics').DefensiveMatchupAssignment[]) => void; readonly updateGamePlanTacticalOverride: (tacticalOverride: MatchTacticalPlan) => void; readonly saveDesignerPlay: (play: import('@/domain/tactics').SavedPlay) => void; readonly deleteDesignerPlay: (playId: string) => void; readonly saveDesignerPlaybook: (playbook: import('@/domain/tactics').Playbook) => void; readonly deleteDesignerPlaybook: (playbookId: string) => void; readonly setCoachLifestyle?: (value: Lifestyle) => void; readonly addRecruitingTarget?: (cycleId: string, recruitId: string, priority: Priority) => void; readonly removeRecruitingTarget?: (recruitId: string) => void; readonly performRecruitingAction?: (cycleId: string, recruitId: string, kind: 'contact'|'pitch'|'visit') => string | null; readonly makeRecruitingOffer?: (cycleId: string, recruitId: string) => string | null; readonly acceptNilOpportunity?: (id: string) => void; readonly requestBoosterSupport?: (id: string) => void; readonly respondToMedia?: (id: string, stance: MediaStance) => void; readonly skipMedia?: (id: string) => void }
