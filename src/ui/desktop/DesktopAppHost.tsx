...
export function DesktopAppHost({ appId, entityDestination, world, actions }: { readonly appId: string; readonly entityDestination?: EntityDestination; readonly world: GameWorld; readonly actions: DesktopAppActions }) {
  ...
  if (appId === 'club') return <ClubPcbPage onFireStaff={actions.fireStaff} onNegotiateStaff={(roleId, staffId) => { const team = getUserTeam(world); if (team !== undefined) actions.negotiateStaffHire(team.id, roleId, staffId) }} world={world} />
  if (appId === 'staff') return <ClubPcbPage initialTab="staff" onFireStaff={actions.fireStaff} onNegotiateStaff={(roleId, staffId) => { const team = getUserTeam(world); if (team !== undefined) actions.negotiateStaffHire(team.id, roleId, staffId) }} world={world} />
