...
import { CoachScreen, DraftScreen, MarketScreen, NilScreen, RecruitingScreen, SalaryScreen, ScheduleScreen, SquadScreen, StaffScreen, StandingsScreen, TacticsScreen, TradeCenterScreen, TrainingScreen } from '@/ui/screens'
...
export interface DesktopAppActions { readonly tacticalPlan: MatchTacticalPlan; readonly openApp: (id: string) => void; readonly playGame: () => void; readonly instantResult: () => void; readonly simulateRemainingGamesToday: () => void; readonly advanceDay: () => void; readonly startNextSeason: () => void; readonly releasePlayer: (teamId: TeamId, playerId: PlayerId) => void; readonly signFreeAgent: (teamId: TeamId, playerId: PlayerId) => void; readonly selectDraftProspect: (draftId: string, playerId: PlayerId) => void; readonly executeTrade?: (proposal: import('@/domain/trade').TradeProposal) => void; readonly purchaseSkill: (id: CoachSkillId) => void; readonly purchasePerk: (id: CoachPerkId) => void; readonly acceptOffer: (id: string) => void; readonly declineOffer: (id: string) => void; readonly setTacticalPlan: (plan: MatchTacticalPlan) => void; readonly resetTacticalPlan: () => void; readonly setTrainingIntensity: (value: TrainingIntensity) => void; readonly setTrainingFocus: (value: TrainingFocus) => void; readonly addRecruitingTarget?: (cycleId: string, recruitId: string, priority: Priority) => void; readonly removeRecruitingTarget?: (recruitId: string) => void; readonly performRecruitingAction?: (cycleId: string, recruitId: string, kind: 'contact'|'pitch'|'visit') => string | null; readonly makeRecruitingOffer?: (cycleId: string, recruitId: string) => string | null; readonly acceptNilOpportunity?: (id: string) => void }
