...
export function DesktopAppHost({ appId, entityDestination, world, actions }: { readonly appId: string; readonly entityDestination?: EntityDestination; readonly world: GameWorld; readonly actions: DesktopAppActions }) {
  ...
  if (appId === 'training') return <TrainingPcbPage world={world} onIntensity={actions.setTrainingIntensity} onFocus={actions.setTrainingFocus} onScheduleSession={actions.scheduleTrainingSession} onCancelSession={actions.cancelTrainingSession} onSaveModule={actions.saveUserTrainingModule} onDeleteModule={actions.deleteUserTrainingModule} onAssignModule={actions.assignTrainingModuleToPlayer} />
