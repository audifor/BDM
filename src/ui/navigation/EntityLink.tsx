...
export function EntityLink({ children, destination, onNavigate }: { readonly children: ReactNode; readonly destination: EntityDestination; readonly onNavigate: (destination: EntityDestination) => void }) {
  const contextMenu = useEntityContextMenu(destination.type === 'player' ? { type: 'player', id: destination.playerId } : destination.type === 'team' ? { type: 'team', id: destination.teamId } : { type: 'competition', id: destination.competitionId })
  const open = (event: MouseEvent<HTMLButtonElement>) => {
    event.stopPropagation()
    onNavigate(destination)
  }
  return <button className="entity-link" {...contextMenu} onClick={open} type="button">{children}</button>
}
