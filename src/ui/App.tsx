...
export function App() {
  ...
  const createStaffOffer = useGameStore((state) => state.createStaffOffer)
  const acceptStaffOffer = useGameStore((state) => state.acceptStaffOffer)
  const declineStaffOffer = useGameStore((state) => state.declineStaffOffer)
  ...
  return (
    <EntityContextMenuProvider onOpenEntity={openEntity} world={world}><DesktopShell
      ...
      widgets={<><DesktopWidgetLayer world={world} onAdvanceDay={advanceDay} onContinue={continueGame} onInstantResult={() => instantResult(tacticalPlanOverride)} onOpenApp={openDesktopApp} onOpenPendingGame={(gameId) => { if (world.games[gameId]?.status === 'scheduled') startMatch(startLiveMatch(tacticalPlanOverride)) }} onPlayGame={() => startMatch(startLiveMatch(tacticalPlanOverride))} onStartNextSeason={startNextSeason} onSimulateUntilDate={simulateUntilDate} /><DesktopCanonicalSurfaceLayer visible={visualQaFixture} world={world} /></>}
