...
export function createCoachJobOpeningForTeam(world: GameWorld, input: { readonly teamId: TeamId; readonly reputationRequirement?: CoachReputationRequirement; readonly id?: string }): { readonly world: GameWorld; readonly opening: CoachJobOpening } {
  ...
  const opening = createCoachJobOpening({ id, teamId: team.id, status: 'open', createdOn: world.currentDate, ...(input.reputationRequirement === undefined ? {} : { reputationRequirement: input.reputationRequirement }) })
...
export function createCoachJobOffer(world: GameWorld, input: { readonly candidacyId: string; readonly id?: string }): { readonly world: GameWorld; readonly offerId: string } {
  ...
  const next=rebuild(world, { candidacies: { ...world.coachJobCandidaciesById, [candidacy.id]: transitioned.candidacy }, offers: { ...world.coachJobOffersById, [id]: { id, jobOpeningId: opening.id, coachId: candidacy.coachId, teamId: opening.teamId, createdOn: world.currentDate, status: 'pending' } } })
  return { world: candidacy.coachId===world.userCoachId?addInboxItem(next,{id:`inbox:career-offer:${id}:${candidacy.coachId}`,coachId:candidacy.coachId,gameDate:world.currentDate,category:'career',priority:'high',title:`Job offer from ${world.teams[opening.teamId]!.name}`,body:`${world.teams[opening.teamId]!.name} has offered you the head coach position.`,status:'unread',action:{type:'coachJobOffer',entityId:id},context:{offerId:id,teamId:opening.teamId}}):next, offerId: id }
}
...
export function acceptCoachJobOffer(world: GameWorld, offerId: string): GameWorld {
  ...
  const next=rebuild(world, { teams, openings, candidacies, offers, employment: { ...world.coachEmploymentByCoachId, [offer.coachId]: appointed.employment }, history: { ...world.coachCareerHistoryByCoachId, [offer.coachId]: appointed.history } })
  const coach=world.coaches[offer.coachId]!, team=world.teams[opening.teamId]!
  return addNewsItem(next,{id:`news:coach-hired:${offer.id}`,gameDate:world.currentDate,category:'career',headline:`${coach.firstName} ${coach.lastName} joins ${team.name}`,body:`${coach.firstName} ${coach.lastName} has been appointed head coach of ${team.name}.`,context:{coachId:coach.id,teamId:team.id,offerId:offer.id}})
}
...
export function runCoachHiringProcessForOpening(world: GameWorld, openingId: string): GameWorld { const opening = requireOpening(world, openingId); const candidates = Object.values(world.coaches).filter((coach) => coach.id !== world.teams[opening.teamId]!.coachId && evaluateCoachJobEligibility(world.coachEmploymentByCoachId[coach.id]!, world.coachReputationProfilesByCoachId[coach.id]!, opening).eligible).sort((a, b) => score(world, b.id) - score(world, a.id) || a.id.localeCompare(b.id)); const coach = candidates[0]; if (coach === undefined) return world; const candidate = identifyCoachCandidate(world, { openingId, coachId: coach.id }); const interviewed = completeCoachInterview(startCoachInterview(candidate.world, candidate.candidacyId), candidate.candidacyId); const offered = createCoachJobOffer(interviewed, { candidacyId: candidate.candidacyId }); return coach.id === world.userCoachId ? offered.world : acceptCoachJobOffer(offered.world, offered.offerId) }

function score(world: GameWorld, coachId: CoachId): number { const values = world.coachReputationProfilesByCoachId[coachId]!.values; return (values.competitive + values.development + values.professional + values.publicStanding) / 4 }
