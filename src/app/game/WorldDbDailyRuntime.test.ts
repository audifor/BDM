...
vi.mock('./advanceGameDay', () => ({ advanceGameDay: vi.fn() }))
...
describe('WorldDbDailyRuntimeSessionV1', () => {
  ...
  it('materializes again after a simulate-until tick resolves gameplay', async () => {
    const next = world('2030-01-02', ['season-1'], true)
    vi.mocked(tickSimulateUntilDate).mockReturnValue({ world: next, event: { type: 'dayAdvanced' } })
