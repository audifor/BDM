...
export function PlayerWorkspaceHeader() {
  ...
  const { model, playerId, setPlayerId, session } = usePlayerWorkspace()
  if (model === null || world === null) return null

  const rosterOptions = rosterPlayerOptions(world)
  const activeCategoryLabel =
    session.activeView === 'attributes'
      ? model.attributes.categories.find((entry) => entry.category === session.attributesCategory)?.label
      : undefined
    ...
    <header className="po-workspace-header" data-ng-region="workspace-header">
      <div className="po-workspace-header__context">
        ...
        {activeCategoryLabel !== undefined && (
          <span className="po-workspace-header__view-context">{activeCategoryLabel}</span>
        )}
