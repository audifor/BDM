...
import { createEconomicObservation, createExchangeRate, createFinancialRegulationAssessment, type EconomicObservation, type ExchangeRate, type FinancialRegulationAssessment } from '@/domain/finance'
...
export interface GameWorldSaveV4 extends GameWorldSaveV3 {
  ...
  readonly financialRegulationAssessments?: readonly FinancialRegulationAssessment[]
...
export function serializeGameWorldV4(world: GameWorld, savedAt: string): SaveGameEnvelopeV4 {
  ...
  return Object.freeze({
    ...
    payload: Object.freeze({
      ...
      financialRegulationAssessments: Object.values(world.financialRegulationAssessmentsById), economicObservations: Object.values(world.economicObservationsById), exchangeRates: Object.values(world.exchangeRatesById),
      financeDecisionProposals: Object.values(world.financeDecisionProposalsById),
...
export function deserializeGameWorldV4(value: unknown): GameWorld {
  ...
  const financialRegulationAssessments = Object.prototype.hasOwnProperty.call(payload, 'financialRegulationAssessments') ? parseFinancialAssessments(payload.financialRegulationAssessments) : []
...
function parseFinanceDecisionProposals(value: unknown): readonly FinanceDecisionProposal[] {
  if (!Array.isArray(value)) throw new TypeError('Save V4 financeDecisionProposals must be an array')
  return Object.freeze(value.map((entry) => {
    const item = record(entry, 'Save V4 FinanceDecisionProposal')
    exactKeys(item, ['id', 'organizationId', 'asOfDate', 'actionType', 'evidence', 'expectedFinancialImpact', 'risks', 'authorityRequired', 'responsibleActor', 'governanceDecisionId', 'status', 'provenance'], 'Save V4 FinanceDecisionProposal')
    if (!Array.isArray(item.evidence) || !Array.isArray(item.risks)) throw new TypeError('Finance proposal evidence and risks must be arrays')
    return createFinanceDecisionProposal({ id: nonEmptyText(item.id, 'Finance proposal id'), organizationId: nonEmptyText(item.organizationId, 'Finance proposal organization'), asOfDate: nonEmptyText(item.asOfDate, 'Finance proposal date') as FinanceDecisionProposal['asOfDate'], actionType: item.actionType as FinanceDecisionProposal['actionType'], evidence: item.evidence as FinanceDecisionProposal['evidence'], expectedFinancialImpact: item.expectedFinancialImpact as FinanceDecisionProposal['expectedFinancialImpact'], risks: item.risks as string[], authorityRequired: item.authorityRequired as FinanceDecisionProposal['authorityRequired'], responsibleActor: item.responsibleActor as FinanceDecisionProposal['responsibleActor'], governanceDecisionId: nullableText(item.governanceDecisionId, 'Finance proposal governance decision'), status: item.status as FinanceDecisionProposal['status'], provenance: parseFinancialSource(item.provenance, 'Finance proposal provenance') })
  }))
}
function parseFinancialAssessments(value: unknown): readonly FinancialRegulationAssessment[] {
  if (!Array.isArray(value)) throw new TypeError('Save V4 financialRegulationAssessments must be an array')
  return Object.freeze(value.map((entry) => {
    const item = record(entry, 'Save V4 FinancialAssessment')
    exactKeys(item, ['id', 'organizationId', 'ruleId', 'ruleVersion', 'authorityOrganizationId', 'competitionId', 'asOfDate', 'periodStartsOn', 'metric', 'comparison', 'currencyCode', 'measuredValue', 'threshold', 'state', 'supportingFactIds', 'provenance', 'consequence'], 'Save V4 FinancialAssessment')
    if (!Array.isArray(item.supportingFactIds)) throw new TypeError('Assessment supporting facts must be an array')
    return createFinancialRegulationAssessment({ id: nonEmptyText(item.id, 'Assessment id'), organizationId: organizationIdFromString(nonEmptyText(item.organizationId, 'Assessment organization')), ruleId: nonEmptyText(item.ruleId, 'Assessment rule'), ruleVersion: nonEmptyText(item.ruleVersion, 'Assessment rule version'), authorityOrganizationId: organizationIdFromString(nonEmptyText(item.authorityOrganizationId, 'Assessment authority')), competitionId: nullableText(item.competitionId, 'Assessment competition'), asOfDate: nonEmptyText(item.asOfDate, 'Assessment date') as FinancialRegulationAssessment['asOfDate'], periodStartsOn: nonEmptyText(item.periodStartsOn, 'Assessment period') as FinancialRegulationAssessment['periodStartsOn'], metric: item.metric as FinancialRegulationAssessment['metric'], comparison: item.comparison as FinancialRegulationAssessment['comparison'], currencyCode: nonEmptyText(item.currencyCode, 'Assessment currency') as FinancialRegulationAssessment['currencyCode'], measuredValue: item.measuredValue === null ? null : integer(item.measuredValue, 'Assessment value'), threshold: integer(item.threshold, 'Assessment threshold'), state: item.state as FinancialRegulationAssessment['state'], supportingFactIds: item.supportingFactIds.map((id) => nonEmptyText(id, 'Assessment supporting fact')), provenance: parseFinancialSource(item.provenance, 'Assessment provenance'), consequence: item.consequence as FinancialRegulationAssessment['consequence'] })
  }))
}
function parseEconomicObservations(value: unknown): readonly EconomicObservation[] {
  if (!Array.isArray(value)) throw new TypeError('Save V4 economicObservations must be an array')
  return Object.freeze(value.map((entry) => {
    const item = record(entry, 'Save V4 EconomicObservation'); const factor = record(item.factor, 'Save V4 EconomicFactor')
    exactKeys(item, ['id', 'kind', 'region', 'currencyCode', 'effectiveOn', 'factor', 'provenance'], 'Save V4 EconomicObservation')
    exactKeys(factor, ['numerator', 'denominator'], 'Save V4 EconomicFactor')
    return createEconomicObservation({ id: nonEmptyText(item.id, 'Observation id'), kind: item.kind as EconomicObservation['kind'], region: nonEmptyText(item.region, 'Observation region'), currencyCode: nonEmptyText(item.currencyCode, 'Observation currency') as EconomicObservation['currencyCode'], effectiveOn: nonEmptyText(item.effectiveOn, 'Observation date') as EconomicObservation['effectiveOn'], factor: { numerator: integer(factor.numerator, 'Factor numerator'), denominator: integer(factor.denominator, 'Factor denominator') }, provenance: parseFinancialSource(item.provenance, 'Observation provenance') })
  }))
}
function parseExchangeRates(value: unknown): readonly ExchangeRate[] {
  if (!Array.isArray(value)) throw new TypeError('Save V4 exchangeRates must be an array')
  return Object.freeze(value.map((entry) => {
    const item = record(entry, 'Save V4 ExchangeRate'); const rate = record(item.rate, 'Save V4 ExchangeFactor')
    exactKeys(item, ['id', 'baseCurrencyCode', 'quoteCurrencyCode', 'effectiveOn', 'rate', 'provenance'], 'Save V4 ExchangeRate')
    exactKeys(rate, ['numerator', 'denominator'], 'Save V4 ExchangeFactor')
    return createExchangeRate({ id: nonEmptyText(item.id, 'Rate id'), baseCurrencyCode: nonEmptyText(item.baseCurrencyCode, 'Rate base') as ExchangeRate['baseCurrencyCode'], quoteCurrencyCode: nonEmptyText(item.quoteCurrencyCode, 'Rate quote') as ExchangeRate['quoteCurrencyCode'], effectiveOn: nonEmptyText(item.effectiveOn, 'Rate date') as ExchangeRate['effectiveOn'], rate: { numerator: integer(rate.numerator, 'Rate numerator'), denominator: integer(rate.denominator, 'Rate denominator') }, provenance: parseFinancialSource(item.provenance, 'Rate provenance') })
  }))
}
...
function parseIndexation(value: unknown): IndexationPolicy {
  const policy = record(value, 'Save V4 IndexationPolicy')
  exactKeys(policy, policy.kind === 'INDEXED' ? ['kind', 'observationId'] : ['kind'], 'Save V4 IndexationPolicy')
  return createIndexationPolicy(policy.kind === 'INDEXED' ? { kind: 'INDEXED', observationId: nonEmptyText(policy.observationId, 'Indexation observationId') } : { kind: policy.kind as 'FIXED' | 'EXTERNALLY_SUPPLIED' })
}
function parseRevenueSources(value: unknown): readonly RevenueSource[] {
  ...
  return Object.freeze(value.map((entry) => {
    ...
    exactKeys(source, ['id', 'organizationId', 'category', 'currencyCode', 'amount', 'startsOn', 'endsOn', 'sourceAuthority', 'status', 'generationPolicy', 'dueDatePolicy', 'teamId', 'organizationSectionId', 'competitionId', 'counterparty', 'paymentSchedule', 'provenance', ...(source.indexation === undefined ? [] : ['indexation'])], 'Save V4 RevenueSource')
...
function parseOperatingCostSources(value: unknown): readonly OperatingCostSource[] {
  ...
  return Object.freeze(value.map((entry) => {
    ...
    exactKeys(source, ['id', 'organizationId', 'category', 'costNature', 'currencyCode', 'amount', 'startsOn', 'endsOn', 'sourceAuthority', 'status', 'generationPolicy', 'dueDatePolicy', 'teamId', 'organizationSectionId', 'competitionId', 'facilityId', 'matchId', 'counterparty', 'paymentSchedule', 'provenance', ...(source.indexation === undefined ? [] : ['indexation'])], 'Save V4 OperatingCostSource')
