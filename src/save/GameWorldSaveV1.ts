...
function readEcosystem(value: unknown) { const v = record(value, 'Sports ecosystem'); return createSportsEcosystem({ id: ecosystemIdFromString(string(v.id, 'Sports ecosystem id')), name: string(v.name, 'Sports ecosystem name'), kind: string(v.kind, 'Sports ecosystem kind') as 'fibaLike', domesticTiers: v.domesticTiers === undefined ? [] : array(v.domesticTiers, 'Domestic tiers').map((tier) => { const t = record(tier, 'Domestic tier'); return { competitionId: competitionIdFromString(string(t.competitionId, 'Domestic tier competition')), level: integer(t.level, 'Domestic tier level') } }), tierMovementRules: v.tierMovementRules === undefined ? [] : array(v.tierMovementRules, 'Tier movement rules').map((rule) => { const r = record(rule, 'Tier movement rule'); return { upperCompetitionId: competitionIdFromString(string(r.upperCompetitionId, 'Tier rule upper competition')), lowerCompetitionId: competitionIdFromString(string(r.lowerCompetitionId, 'Tier rule lower competition')), exchangeCount: integer(r.exchangeCount, 'Tier rule exchange count') } }) }) }
