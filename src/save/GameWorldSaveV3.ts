...
import { createGovernanceAppointment, createGovernanceAuthorityGrant, createGovernanceBody, createGovernanceDecision, createGovernanceDecisionEvent, createGovernanceDecisionParticipationGrant, createGovernanceExpectationPeriod, createGovernanceExternalRelationship, createGovernanceInstitution, createGovernanceJobSecurityTransition, createGovernanceManagerEvaluation, createGovernanceManagerEvaluationPeriod, createGovernanceObjective, GOVERNANCE_BODY_KINDS, GOVERNANCE_DECISION_EVENT_KINDS, GOVERNANCE_DECISION_RIGHTS, GOVERNANCE_DECISION_TYPES, GOVERNANCE_EXPECTATION_HORIZONS, GOVERNANCE_EXTERNAL_RELATIONSHIP_TYPES, GOVERNANCE_FACTOR_DIRECTIONS, GOVERNANCE_FACTOR_STATUSES, GOVERNANCE_JOB_SECURITY_STATES, GOVERNANCE_MANAGER_EVALUATION_FACTOR_KINDS, GOVERNANCE_OBJECTIVE_COMPARISONS, GOVERNANCE_OBJECTIVE_FAMILIES, GOVERNANCE_OBJECTIVE_METRICS, GOVERNANCE_ROLES, GOVERNANCE_UNIVERSES, type GovernanceAppointment, type GovernanceAuthorityGrant, type GovernanceBody, type GovernanceDecision, type GovernanceDecisionEvent, type GovernanceDecisionParticipationGrant, type GovernanceExpectationPeriod, type GovernanceExternalRelationship, type GovernanceInstitution, type GovernanceJobSecurityTransition, type GovernanceManagerEvaluation, type GovernanceManagerEvaluationFactorSource, type GovernanceManagerEvaluationPeriod, type GovernanceObjective } from '@/domain/governance'
import { createGovernanceMeeting, createGovernanceMeetingAgendaItem, createGovernanceMeetingEvent, createGovernanceMeetingParticipant, GOVERNANCE_MEETING_ATTENDANCES, GOVERNANCE_MEETING_CATEGORIES, GOVERNANCE_MEETING_EVENT_KINDS, GOVERNANCE_MEETING_PARTICIPANT_ROLES, type GovernanceInteractionParty, type GovernanceMeeting, type GovernanceMeetingAgendaItem, type GovernanceMeetingEvent, type GovernanceMeetingParticipant } from '@/domain/governance'
import { createGovernanceRequest, createGovernanceRequestEvent, GOVERNANCE_REQUEST_CATEGORIES, GOVERNANCE_REQUEST_EVENT_KINDS, type GovernanceRequest, type GovernanceRequestEvent } from '@/domain/governance'
import { createGovernanceCommitment, createGovernanceCommitmentEvent, type GovernanceCommitment, type GovernanceCommitmentEvent } from '@/domain/governance'
import { createGovernanceUniverseProfile, type GovernanceUniverseProfile } from '@/domain/governance'
import { createCollectiveInstitutionAffiliation, createCollectiveParticipantAffiliation, createSupportContribution, createSupporterRelationship, createSupportFundingPledge, createSupportFundingPledgeEvent, COLLECTIVE_AFFILIATION_KINDS, COLLECTIVE_PARTICIPANT_KINDS, DONOR_PATTERNS, INSTITUTIONAL_SUPPORT_SCOPES, SUPPORTER_RELATIONSHIP_KINDS, SUPPORT_CONTRIBUTION_MEDIA, SUPPORT_FUNDING_PLEDGE_EVENT_KINDS, SUPPORT_FUNDING_PLEDGE_SCHEDULES, SUPPORT_FUNDING_RESTRICTIONS, SUPPORT_FUNDING_SCOPES, type CollectiveInstitutionAffiliation, type CollectiveParticipantAffiliation, type SupportContribution, type SupportFundingPledge, type SupportFundingPledgeEvent, type SupporterRelationship } from '@/domain/supporters'
...
function v3Payload(payload: GameWorldSaveV2, world: GameWorld): GameWorldSaveV3 {
  return {
    ...
    staffCareerRuntime: {
      ...
      governanceUniverseProfiles: Object.values(world.governanceUniverseProfilesById),
      ...
      governanceDecisionParticipationGrants: Object.values(world.governanceDecisionParticipationGrantsById),
      governanceDecisions: Object.values(world.governanceDecisionsById),
      governanceDecisionEvents: Object.values(world.governanceDecisionEventsById),
      governanceMeetings: values(world.governanceMeetingsById), governanceMeetingParticipants: values(world.governanceMeetingParticipantsById), governanceMeetingAgendaItems: values(world.governanceMeetingAgendaItemsById), governanceMeetingEvents: values(world.governanceMeetingEventsById), governanceRequests: values(world.governanceRequestsById), governanceRequestEvents: values(world.governanceRequestEventsById), governanceCommitments: values(world.governanceCommitmentsById), governanceCommitmentEvents: values(world.governanceCommitmentEventsById),
      supporterRelationships: values(world.supporterRelationshipsById), collectiveInstitutionAffiliations: values(world.collectiveInstitutionAffiliationsById), collectiveParticipantAffiliations: values(world.collectiveParticipantAffiliationsById),
      supportFundingPledges: values(world.supportFundingPledgesById), supportFundingPledgeEvents: values(world.supportFundingPledgeEventsById), supportContributions: values(world.supportContributionsById),
    },
  }
}
...
function parseStaffCareerRuntimeV3(value: unknown): {
  ...
  readonly governanceMeetings: readonly GovernanceMeeting[]; readonly governanceMeetingParticipants: readonly GovernanceMeetingParticipant[]; readonly governanceMeetingAgendaItems: readonly GovernanceMeetingAgendaItem[]; readonly governanceMeetingEvents: readonly GovernanceMeetingEvent[]; readonly governanceRequests: readonly GovernanceRequest[]; readonly governanceRequestEvents: readonly GovernanceRequestEvent[]; readonly governanceCommitments: readonly GovernanceCommitment[]; readonly governanceCommitmentEvents: readonly GovernanceCommitmentEvent[]
  readonly supporterRelationships: readonly SupporterRelationship[]; readonly collectiveInstitutionAffiliations: readonly CollectiveInstitutionAffiliation[]; readonly collectiveParticipantAffiliations: readonly CollectiveParticipantAffiliation[]; readonly supportFundingPledges: readonly SupportFundingPledge[]; readonly supportFundingPledgeEvents: readonly SupportFundingPledgeEvent[]; readonly supportContributions: readonly SupportContribution[]
} {
  ...
  const governanceUniverseProfilesPayload = rawRuntime.governanceUniverseProfiles
  ...
  const collectiveInstitutionAffiliationsPayload = runtime.collectiveInstitutionAffiliations
  const collectiveParticipantAffiliationsPayload = runtime.collectiveParticipantAffiliations
  ...
  const governanceUniverseProfiles = governanceUniverseProfilesPayload === undefined ? [] : array(governanceUniverseProfilesPayload, 'Governance universe profiles').map(parseGovernanceUniverseProfileV3)
  ...
  const governanceDecisionParticipationGrants = runtime.governanceDecisionParticipationGrants === undefined ? [] : array(runtime.governanceDecisionParticipationGrants, 'Governance decision participation grants').map(parseGovernanceDecisionParticipationGrantV3)
  const governanceDecisions = runtime.governanceDecisions === undefined ? [] : array(runtime.governanceDecisions, 'Governance decisions').map(parseGovernanceDecisionV3)
  const governanceDecisionEvents = runtime.governanceDecisionEvents === undefined ? [] : array(runtime.governanceDecisionEvents, 'Governance decision events').map(parseGovernanceDecisionEventV3)
  const governanceMeetings = runtime.governanceMeetings === undefined ? [] : array(runtime.governanceMeetings, 'Governance meetings').map(parseGovernanceMeetingV3)
  const governanceMeetingParticipants = runtime.governanceMeetingParticipants === undefined ? [] : array(runtime.governanceMeetingParticipants, 'Governance meeting participants').map(parseGovernanceMeetingParticipantV3)
  const governanceMeetingAgendaItems = runtime.governanceMeetingAgendaItems === undefined ? [] : array(runtime.governanceMeetingAgendaItems, 'Governance meeting agenda items').map(parseGovernanceMeetingAgendaItemV3)
  const governanceMeetingEvents = runtime.governanceMeetingEvents === undefined ? [] : array(runtime.governanceMeetingEvents, 'Governance meeting events').map(parseGovernanceMeetingEventV3)
  const governanceRequests = runtime.governanceRequests === undefined ? [] : array(runtime.governanceRequests, 'Governance requests').map(parseGovernanceRequestV3)
  const governanceRequestEvents = runtime.governanceRequestEvents === undefined ? [] : array(runtime.governanceRequestEvents, 'Governance request events').map(parseGovernanceRequestEventV3)
  const governanceCommitments = runtime.governanceCommitments === undefined ? [] : array(runtime.governanceCommitments, 'Governance commitments').map(parseGovernanceCommitmentV3)
  const governanceCommitmentEvents = runtime.governanceCommitmentEvents === undefined ? [] : array(runtime.governanceCommitmentEvents, 'Governance commitment events').map(parseGovernanceCommitmentEventV3)
  const supporterRelationships = supporterRelationshipsPayload === undefined ? [] : array(supporterRelationshipsPayload, 'Supporter relationships').map(parseSupporterRelationshipV3)
  const collectiveInstitutionAffiliations = collectiveInstitutionAffiliationsPayload === undefined ? [] : array(collectiveInstitutionAffiliationsPayload, 'Collective institution affiliations').map(parseCollectiveInstitutionAffiliationV3)
  const collectiveParticipantAffiliations = collectiveParticipantAffiliationsPayload === undefined ? [] : array(collectiveParticipantAffiliationsPayload, 'Collective participant affiliations').map(parseCollectiveParticipantAffiliationV3)
  const supportFundingPledges = supportFundingPledgesPayload === undefined ? [] : array(supportFundingPledgesPayload, 'Support funding pledges').map(parseSupportFundingPledgeV3)
  const supportFundingPledgeEvents = supportFundingPledgeEventsPayload === undefined ? [] : array(supportFundingPledgeEventsPayload, 'Support funding pledge events').map(parseSupportFundingPledgeEventV3)
  const supportContributions = supportContributionsPayload === undefined ? [] : array(supportContributionsPayload, 'Support contributions').map(parseSupportContributionV3)

  return { staffEmploymentByStaffId, staffCareerHistoryByStaffId, staffJobOpenings, staffJobCandidacies, staffInterviewsByCandidacyId, staffJobOffers, staffContracts, staffReputationProfilesByStaffId, staffPoliticalCases, staffPoliticalActions, staffPoliticalAlliances, staffPoliticalFactions, governanceInstitutions, governanceUniverseProfiles, governanceBodies, governanceAppointments, governanceAuthorityGrants, governanceExternalRelationships, governanceExpectationPeriods, governanceObjectives, governanceManagerEvaluationPeriods, governanceManagerEvaluations, governanceJobSecurityTransitions, governanceDecisionParticipationGrants, governanceDecisions, governanceDecisionEvents, governanceMeetings, governanceMeetingParticipants, governanceMeetingAgendaItems, governanceMeetingEvents, governanceRequests, governanceRequestEvents, governanceCommitments, governanceCommitmentEvents, supporterRelationships, collectiveInstitutionAffiliations, collectiveParticipantAffiliations, supportFundingPledges, supportFundingPledgeEvents, supportContributions }
}

function supportBoolean(value: unknown, label: string): boolean { if (typeof value !== 'boolean') throw new TypeError(`${label} must be boolean`); return value }
function parseSupporterActorV3(value: unknown) { const item = record(value, 'Supporter actor V3'); assertExactKeys(item, ['kind', 'id'], 'Supporter actor V3'); return { kind: enumValue(item.kind, ['COACH', 'STAFF', 'EXTERNAL'], 'Supporter actor kind') as 'COACH' | 'STAFF' | 'EXTERNAL', id: text(item.id, 'Supporter actor ID') } }
function parseSupporterRelationshipV3(value: unknown): SupporterRelationship { const item = record(value, 'Supporter relationship V3'); assertExactKeys(item, ['id', 'actor', 'kind', 'institutionId', 'scope', 'programTeamIds', 'startedOn', ...(item.endedOn === undefined ? [] : ['endedOn']), ...(item.donorPattern === undefined ? [] : ['donorPattern']), 'restricted'], 'Supporter relationship V3'); return createSupporterRelationship({ id: text(item.id, 'Supporter relationship ID'), actor: parseSupporterActorV3(item.actor), kind: enumValue(item.kind, SUPPORTER_RELATIONSHIP_KINDS, 'Supporter relationship kind') as SupporterRelationship['kind'], institutionId: text(item.institutionId, 'Supporter relationship institution'), scope: enumValue(item.scope, INSTITUTIONAL_SUPPORT_SCOPES, 'Supporter relationship scope') as SupporterRelationship['scope'], programTeamIds: array(item.programTeamIds, 'Supporter relationship programs').map((id) => teamIdFromString(text(id, 'Supporter relationship program'))), startedOn: parseGameDate(text(item.startedOn, 'Supporter relationship startedOn')), ...(item.endedOn === undefined ? {} : { endedOn: parseGameDate(text(item.endedOn, 'Supporter relationship endedOn')) }), ...(item.donorPattern === undefined ? {} : { donorPattern: enumValue(item.donorPattern, DONOR_PATTERNS, 'Supporter donor pattern') as SupporterRelationship['donorPattern'] }), restricted: supportBoolean(item.restricted, 'Supporter relationship restricted') }) }
function parseCollectiveInstitutionAffiliationV3(value: unknown): CollectiveInstitutionAffiliation { const item = record(value, 'Collective institution affiliation V3'); assertExactKeys(item, ['id', 'collectiveId', 'institutionId', 'kind', 'scope', 'programTeamIds', 'startedOn', ...(item.endedOn === undefined ? [] : ['endedOn'])], 'Collective institution affiliation V3'); return createCollectiveInstitutionAffiliation({ id: text(item.id, 'Collective affiliation ID'), collectiveId: text(item.collectiveId, 'Collective affiliation collective'), institutionId: text(item.institutionId, 'Collective affiliation institution'), kind: enumValue(item.kind, COLLECTIVE_AFFILIATION_KINDS, 'Collective affiliation kind') as CollectiveInstitutionAffiliation['kind'], scope: enumValue(item.scope, INSTITUTIONAL_SUPPORT_SCOPES, 'Collective affiliation scope') as CollectiveInstitutionAffiliation['scope'], programTeamIds: array(item.programTeamIds, 'Collective affiliation programs').map((id) => teamIdFromString(text(id, 'Collective affiliation program'))), startedOn: parseGameDate(text(item.startedOn, 'Collective affiliation startedOn')), ...(item.endedOn === undefined ? {} : { endedOn: parseGameDate(text(item.endedOn, 'Collective affiliation endedOn')) }) }) }
function parseCollectiveParticipantAffiliationV3(value: unknown): CollectiveParticipantAffiliation { const item = record(value, 'Collective participant affiliation V3'); assertExactKeys(item, ['id', 'collectiveId', 'actor', 'kind', 'startedOn', ...(item.endedOn === undefined ? [] : ['endedOn'])], 'Collective participant affiliation V3'); return createCollectiveParticipantAffiliation({ id: text(item.id, 'Collective participant ID'), collectiveId: text(item.collectiveId, 'Collective participant collective'), actor: parseSupporterActorV3(item.actor), kind: enumValue(item.kind, COLLECTIVE_PARTICIPANT_KINDS, 'Collective participant kind') as CollectiveParticipantAffiliation['kind'], startedOn: parseGameDate(text(item.startedOn, 'Collective participant startedOn')), ...(item.endedOn === undefined ? {} : { endedOn: parseGameDate(text(item.endedOn, 'Collective participant endedOn')) }) }) }
function parseSupportFundingTargetV3(value: unknown) { const item = record(value, 'Support funding target V3'); assertExactKeys(item, ['institutionId', 'scope', 'programTeamIds', ...(item.collectiveId === undefined ? [] : ['collectiveId'])], 'Support funding target V3'); return { institutionId: text(item.institutionId, 'Support funding target institution'), scope: enumValue(item.scope, SUPPORT_FUNDING_SCOPES, 'Support funding target scope') as never, programTeamIds: array(item.programTeamIds, 'Support funding target programs').map((id) => teamIdFromString(text(id, 'Support funding target program'))), ...(item.collectiveId === undefined ? {} : { collectiveId: text(item.collectiveId, 'Support funding target collective') }) } }
function parseSupportFundingPledgeV3(value: unknown): SupportFundingPledge { const item = record(value, 'Support funding pledge V3'); assertExactKeys(item, ['id', 'contributor', 'target', 'effectiveOn', 'schedule', 'medium', 'restriction', ...(item.supporterRelationshipId === undefined ? [] : ['supporterRelationshipId']), ...(item.endsOn === undefined ? [] : ['endsOn']), ...(item.amountMinorUnits === undefined ? [] : ['amountMinorUnits']), ...(item.currencyCode === undefined ? [] : ['currencyCode']), ...(item.inKindDescription === undefined ? [] : ['inKindDescription']), ...(item.purpose === undefined ? [] : ['purpose'])], 'Support funding pledge V3'); return createSupportFundingPledge({ id: text(item.id, 'Support funding pledge ID'), contributor: parseSupporterActorV3(item.contributor), target: parseSupportFundingTargetV3(item.target), effectiveOn: parseGameDate(text(item.effectiveOn, 'Support funding pledge effective date')), schedule: enumValue(item.schedule, SUPPORT_FUNDING_PLEDGE_SCHEDULES, 'Support funding pledge schedule') as never, medium: enumValue(item.medium, SUPPORT_CONTRIBUTION_MEDIA, 'Support funding pledge medium') as never, restriction: enumValue(item.restriction, SUPPORT_FUNDING_RESTRICTIONS, 'Support funding pledge restriction') as never, ...(item.supporterRelationshipId === undefined ? {} : { supporterRelationshipId: text(item.supporterRelationshipId, 'Support funding pledge relationship') }), ...(item.endsOn === undefined ? {} : { endsOn: parseGameDate(text(item.endsOn, 'Support funding pledge end date')) }), ...(item.amountMinorUnits === undefined ? {} : { amountMinorUnits: integer(item.amountMinorUnits, 'Support funding pledge amount') }), ...(item.currencyCode === undefined ? {} : { currencyCode: text(item.currencyCode, 'Support funding pledge currency') }), ...(item.inKindDescription === undefined ? {} : { inKindDescription: text(item.inKindDescription, 'Support funding pledge in-kind description') }), ...(item.purpose === undefined ? {} : { purpose: text(item.purpose, 'Support funding pledge purpose') }) }) }
function parseSupportFundingPledgeEventV3(value: unknown): SupportFundingPledgeEvent { const item = record(value, 'Support funding pledge event V3'); assertExactKeys(item, ['id', 'pledgeId', 'kind', 'effectiveOn'], 'Support funding pledge event V3'); return createSupportFundingPledgeEvent({ id: text(item.id, 'Support funding pledge event ID'), pledgeId: text(item.pledgeId, 'Support funding pledge event pledge'), kind: enumValue(item.kind, SUPPORT_FUNDING_PLEDGE_EVENT_KINDS, 'Support funding pledge event kind') as never, effectiveOn: parseGameDate(text(item.effectiveOn, 'Support funding pledge event date')) }) }
function parseSupportContributionV3(value: unknown): SupportContribution { const item = record(value, 'Support contribution V3'); assertExactKeys(item, ['id', 'contributor', 'target', 'effectiveOn', 'medium', 'restriction', ...(item.supporterRelationshipId === undefined ? [] : ['supporterRelationshipId']), ...(item.pledgeId === undefined ? [] : ['pledgeId']), ...(item.amountMinorUnits === undefined ? [] : ['amountMinorUnits']), ...(item.currencyCode === undefined ? [] : ['currencyCode']), ...(item.inKindDescription === undefined ? [] : ['inKindDescription']), ...(item.declaredValueMinorUnits === undefined ? [] : ['declaredValueMinorUnits']), ...(item.declaredValueCurrencyCode === undefined ? [] : ['declaredValueCurrencyCode']), ...(item.purpose === undefined ? [] : ['purpose'])], 'Support contribution V3'); return createSupportContribution({ id: text(item.id, 'Support contribution ID'), contributor: parseSupporterActorV3(item.contributor), target: parseSupportFundingTargetV3(item.target), effectiveOn: parseGameDate(text(item.effectiveOn, 'Support contribution effective date')), medium: enumValue(item.medium, SUPPORT_CONTRIBUTION_MEDIA, 'Support contribution medium') as never, restriction: enumValue(item.restriction, SUPPORT_FUNDING_RESTRICTIONS, 'Support contribution restriction') as never, ...(item.supporterRelationshipId === undefined ? {} : { supporterRelationshipId: text(item.supporterRelationshipId, 'Support contribution relationship') }), ...(item.pledgeId === undefined ? {} : { pledgeId: text(item.pledgeId, 'Support contribution pledge') }), ...(item.amountMinorUnits === undefined ? {} : { amountMinorUnits: integer(item.amountMinorUnits, 'Support contribution amount') }), ...(item.currencyCode === undefined ? {} : { currencyCode: text(item.currencyCode, 'Support contribution currency') }), ...(item.inKindDescription === undefined ? {} : { inKindDescription: text(item.inKindDescription, 'Support contribution in-kind description') }), ...(item.declaredValueMinorUnits === undefined ? {} : { declaredValueMinorUnits: integer(item.declaredValueMinorUnits, 'Support contribution declared value') }), ...(item.declaredValueCurrencyCode === undefined ? {} : { declaredValueCurrencyCode: text(item.declaredValueCurrencyCode, 'Support contribution declared value currency') }), ...(item.purpose === undefined ? {} : { purpose: text(item.purpose, 'Support contribution purpose') }) }) }
...
function parseGovernanceMeetingV3(value: unknown): GovernanceMeeting {
  ...
  assertExactKeys(item, ['id', 'institutionId', 'conveningBodyId', 'category', 'title', 'createdOn'], 'Governance meeting V3')
  return createGovernanceMeeting({ id: text(item.id, 'Governance meeting ID'), institutionId: text(item.institutionId, 'Governance meeting institution'), conveningBodyId: text(item.conveningBodyId, 'Governance meeting body'), category: enumValue(item.category, GOVERNANCE_MEETING_CATEGORIES, 'Governance meeting category') as GovernanceMeeting['category'], title: text(item.title, 'Governance meeting title'), createdOn: parseGameDate(text(item.createdOn, 'Governance meeting createdOn')) })
}
function parseGovernanceInteractionPartyV3(value: unknown): GovernanceInteractionParty {
  const item = record(value, 'Governance interaction party V3')
  const kind = text(item.kind, 'Governance interaction party kind')
  if (kind === 'BODY') { assertExactKeys(item, ['kind', 'bodyId'], 'Governance body interaction party V3'); return { kind, bodyId: text(item.bodyId, 'Governance interaction party body') } }
  if (kind === 'APPOINTMENT') { assertExactKeys(item, ['kind', 'appointmentId'], 'Governance appointment interaction party V3'); return { kind, appointmentId: text(item.appointmentId, 'Governance interaction party appointment') } }
  if (kind === 'ACTOR') { assertExactKeys(item, ['kind', 'actor'], 'Governance actor interaction party V3'); const actor = record(item.actor, 'Governance interaction actor V3'); assertExactKeys(actor, ['kind', 'id'], 'Governance interaction actor V3'); return { kind, actor: { kind: enumValue(actor.kind, ['COACH', 'STAFF'], 'Governance interaction actor kind') as 'COACH' | 'STAFF', id: text(actor.id, 'Governance interaction actor ID') } } }
  throw new TypeError('Unknown governance interaction party kind')
}
function parseGovernanceMeetingParticipantV3(value: unknown): GovernanceMeetingParticipant {
  const item = record(value, 'Governance meeting participant V3')
  assertExactKeys(item, ['id', 'meetingId', 'party', 'role', 'attendance'], 'Governance meeting participant V3')
  return createGovernanceMeetingParticipant({ id: text(item.id, 'Governance meeting participant ID'), meetingId: text(item.meetingId, 'Governance meeting participant meeting'), party: parseGovernanceInteractionPartyV3(item.party), role: enumValue(item.role, GOVERNANCE_MEETING_PARTICIPANT_ROLES, 'Governance meeting participant role') as GovernanceMeetingParticipant['role'], attendance: enumValue(item.attendance, GOVERNANCE_MEETING_ATTENDANCES, 'Governance meeting participant attendance') as GovernanceMeetingParticipant['attendance'] })
}
function parseGovernanceMeetingAgendaItemV3(value: unknown): GovernanceMeetingAgendaItem {
  const item = record(value, 'Governance meeting agenda item V3')
  assertExactKeys(item, ['id', 'meetingId', 'sequence', 'subject'], 'Governance meeting agenda item V3')
  const raw = record(item.subject, 'Governance meeting agenda subject V3'), kind = text(raw.kind, 'Governance meeting agenda subject kind')
  const subject = kind === 'GOVERNANCE_DECISION' ? (assertExactKeys(raw, ['kind', 'decisionId'], 'Governance decision agenda subject V3'), { kind, decisionId: text(raw.decisionId, 'Governance agenda decision') } as const) : kind === 'GOVERNANCE_OBJECTIVE' ? (assertExactKeys(raw, ['kind', 'objectiveId'], 'Governance objective agenda subject V3'), { kind, objectiveId: text(raw.objectiveId, 'Governance agenda objective') } as const) : kind === 'GOVERNANCE_MANAGER_EVALUATION' ? (assertExactKeys(raw, ['kind', 'evaluationId'], 'Governance manager evaluation agenda subject V3'), { kind, evaluationId: text(raw.evaluationId, 'Governance agenda evaluation') } as const) : kind === 'GOVERNANCE_REQUEST' ? (assertExactKeys(raw, ['kind', 'requestId'], 'Governance request agenda subject V3'), { kind, requestId: text(raw.requestId, 'Governance agenda request') } as const) : kind === 'GOVERNANCE_COMMITMENT' ? (assertExactKeys(raw, ['kind', 'commitmentId'], 'Governance commitment agenda subject V3'), { kind, commitmentId: text(raw.commitmentId, 'Governance agenda commitment') } as const) : kind === 'INSTITUTIONAL_TOPIC' ? (assertExactKeys(raw, ['kind', 'topic'], 'Institutional topic agenda subject V3'), { kind, topic: text(raw.topic, 'Governance agenda topic') } as const) : (() => { throw new TypeError('Unknown governance meeting agenda subject kind') })()
  return createGovernanceMeetingAgendaItem({ id: text(item.id, 'Governance meeting agenda item ID'), meetingId: text(item.meetingId, 'Governance meeting agenda meeting'), sequence: integer(item.sequence, 'Governance meeting agenda sequence'), subject })
}
function parseGovernanceMeetingEventV3(value: unknown): GovernanceMeetingEvent {
  const item = record(value, 'Governance meeting event V3'), kind = text(item.kind, 'Governance meeting event kind')
  if (kind === 'SCHEDULED' || kind === 'RESCHEDULED') { assertExactKeys(item, ['id', 'meetingId', 'kind', 'effectiveOn', 'scheduledFor'], 'Scheduled governance meeting event V3'); return createGovernanceMeetingEvent({ id: text(item.id, 'Governance meeting event ID'), meetingId: text(item.meetingId, 'Governance meeting event meeting'), kind, effectiveOn: parseGameDate(text(item.effectiveOn, 'Governance meeting event effectiveOn')), scheduledFor: parseGameDate(text(item.scheduledFor, 'Governance meeting event scheduledFor')) }) }
  if (kind === 'HELD' || kind === 'CANCELLED') { assertExactKeys(item, ['id', 'meetingId', 'kind', 'effectiveOn'], 'Terminal governance meeting event V3'); return createGovernanceMeetingEvent({ id: text(item.id, 'Governance meeting event ID'), meetingId: text(item.meetingId, 'Governance meeting event meeting'), kind, effectiveOn: parseGameDate(text(item.effectiveOn, 'Governance meeting event effectiveOn')) }) }
...
function parseGovernanceRequestV3(value: unknown): GovernanceRequest {
  const item = record(value, 'Governance request V3'); assertExactKeys(item, ['id', 'institutionId', 'issuer', 'recipient', 'category', 'summary', ...(item.dueOn === undefined ? [] : ['dueOn']), 'origin'], 'Governance request V3')
  const origin = record(item.origin, 'Governance request origin V3'), kind = text(origin.kind, 'Governance request origin kind')
  const parsedOrigin = kind === 'STANDALONE' ? (assertExactKeys(origin, ['kind'], 'Standalone governance request origin V3'), { kind } as const) : kind === 'MEETING' ? (assertExactKeys(origin, ['kind', 'meetingId', ...(origin.agendaItemId === undefined ? [] : ['agendaItemId'])], 'Meeting governance request origin V3'), { kind, meetingId: text(origin.meetingId, 'Governance request origin meeting'), ...(origin.agendaItemId === undefined ? {} : { agendaItemId: text(origin.agendaItemId, 'Governance request origin agenda') }) } as const) : (() => { throw new TypeError('Unknown governance request origin kind') })()
  return createGovernanceRequest({ id: text(item.id, 'Governance request ID'), institutionId: text(item.institutionId, 'Governance request institution'), issuer: parseGovernanceInteractionPartyV3(item.issuer), recipient: parseGovernanceInteractionPartyV3(item.recipient), category: enumValue(item.category, GOVERNANCE_REQUEST_CATEGORIES, 'Governance request category') as GovernanceRequest['category'], summary: text(item.summary, 'Governance request summary'), ...(item.dueOn === undefined ? {} : { dueOn: parseGameDate(text(item.dueOn, 'Governance request dueOn')) }), origin: parsedOrigin })
}
function parseGovernanceRequestEventV3(value: unknown): GovernanceRequestEvent {
  const item = record(value, 'Governance request event V3'), kind = enumValue(item.kind, GOVERNANCE_REQUEST_EVENT_KINDS, 'Governance request event kind')
  if (kind === 'FULFILLED') { assertExactKeys(item, ['id', 'requestId', 'kind', 'effectiveOn', 'actor', ...(item.evidence === undefined ? [] : ['evidence'])], 'Fulfilled governance request event V3'); const evidence = item.evidence === undefined ? undefined : (() => { const raw = record(item.evidence, 'Governance request fulfillment evidence V3'), evidenceKind = text(raw.kind, 'Governance request evidence kind'); if (evidenceKind === 'GOVERNANCE_DECISION') { assertExactKeys(raw, ['kind', 'decisionId'], 'Governance request decision evidence V3'); return { kind: 'GOVERNANCE_DECISION' as const, decisionId: text(raw.decisionId, 'Governance request evidence decision') } } if (evidenceKind === 'GOVERNANCE_COMMITMENT') { assertExactKeys(raw, ['kind', 'commitmentId'], 'Governance request commitment evidence V3'); return { kind: 'GOVERNANCE_COMMITMENT' as const, commitmentId: text(raw.commitmentId, 'Governance request evidence commitment') } } throw new TypeError('Unknown governance request evidence kind') })(); return createGovernanceRequestEvent({ id: text(item.id, 'Governance request event ID'), requestId: text(item.requestId, 'Governance request event request'), kind, effectiveOn: parseGameDate(text(item.effectiveOn, 'Governance request event effectiveOn')), actor: parseGovernanceInteractionPartyV3(item.actor), ...(evidence === undefined ? {} : { evidence }) }) }
  assertExactKeys(item, ['id', 'requestId', 'kind', 'effectiveOn', 'actor'], 'Governance request event V3')
  return createGovernanceRequestEvent({ id: text(item.id, 'Governance request event ID'), requestId: text(item.requestId, 'Governance request event request'), kind: kind as Exclude<GovernanceRequestEvent['kind'], 'FULFILLED'>, effectiveOn: parseGameDate(text(item.effectiveOn, 'Governance request event effectiveOn')), actor: parseGovernanceInteractionPartyV3(item.actor) })
}
function parseGovernanceCommitmentV3(value: unknown): GovernanceCommitment {
  const item = record(value, 'Governance commitment V3'); assertExactKeys(item, ['id', 'institutionId', 'promisor', 'beneficiary', 'summary', ...(item.dueOn === undefined ? [] : ['dueOn']), 'provenance'], 'Governance commitment V3')
  const raw = record(item.provenance, 'Governance commitment provenance V3'), kind = text(raw.kind, 'Governance commitment provenance kind')
  const provenance = kind === 'STANDALONE' ? (assertExactKeys(raw, ['kind'], 'Standalone governance commitment provenance V3'), { kind } as const) : kind === 'REQUEST' ? (assertExactKeys(raw, ['kind', 'requestId'], 'Request governance commitment provenance V3'), { kind, requestId: text(raw.requestId, 'Commitment provenance request') } as const) : kind === 'GOVERNANCE_DECISION' ? (assertExactKeys(raw, ['kind', 'decisionId'], 'Decision governance commitment provenance V3'), { kind, decisionId: text(raw.decisionId, 'Commitment provenance decision') } as const) : kind === 'MEETING' ? (assertExactKeys(raw, ['kind', 'meetingId', ...(raw.agendaItemId === undefined ? [] : ['agendaItemId'])], 'Meeting governance commitment provenance V3'), { kind, meetingId: text(raw.meetingId, 'Commitment provenance meeting'), ...(raw.agendaItemId === undefined ? {} : { agendaItemId: text(raw.agendaItemId, 'Commitment provenance agenda') }) } as const) : (() => { throw new TypeError('Unknown governance commitment provenance kind') })()
  return createGovernanceCommitment({ id: text(item.id, 'Governance commitment ID'), institutionId: text(item.institutionId, 'Governance commitment institution'), promisor: parseGovernanceInteractionPartyV3(item.promisor), beneficiary: parseGovernanceInteractionPartyV3(item.beneficiary), summary: text(item.summary, 'Governance commitment summary'), ...(item.dueOn === undefined ? {} : { dueOn: parseGameDate(text(item.dueOn, 'Governance commitment dueOn')) }), provenance })
}
function parseGovernanceCommitmentEventV3(value: unknown): GovernanceCommitmentEvent {
  const item = record(value, 'Governance commitment event V3'), kind = text(item.kind, 'Governance commitment event kind')
  const common = { id: text(item.id, 'Governance commitment event ID'), commitmentId: text(item.commitmentId, 'Governance commitment event commitment'), effectiveOn: parseGameDate(text(item.effectiveOn, 'Governance commitment event effectiveOn')) }
  if (kind === 'MADE' || kind === 'WITHDRAWN') { assertExactKeys(item, ['id', 'commitmentId', 'kind', 'effectiveOn', 'actor'], 'Governance commitment actor event V3'); return createGovernanceCommitmentEvent({ ...common, kind, actor: parseGovernanceInteractionPartyV3(item.actor) }) }
  if (kind === 'BREACHED') { assertExactKeys(item, ['id', 'commitmentId', 'kind', 'effectiveOn'], 'Governance commitment breach event V3'); return createGovernanceCommitmentEvent({ ...common, kind }) }
  if (kind === 'SUPERSEDED') { assertExactKeys(item, ['id', 'commitmentId', 'kind', 'effectiveOn', 'successorCommitmentId'], 'Governance commitment supersession event V3'); return createGovernanceCommitmentEvent({ ...common, kind, successorCommitmentId: text(item.successorCommitmentId, 'Commitment successor') }) }
  if (kind === 'FULFILLED') { assertExactKeys(item, ['id', 'commitmentId', 'kind', 'effectiveOn', 'actor', ...(item.evidence === undefined ? [] : ['evidence'])], 'Governance commitment fulfilled event V3'); const evidence = item.evidence === undefined ? undefined : (() => { const evidence = record(item.evidence, 'Governance commitment fulfillment evidence V3'); assertExactKeys(evidence, ['kind', 'decisionId'], 'Governance commitment fulfillment evidence V3'); if (text(evidence.kind, 'Governance commitment evidence kind') !== 'GOVERNANCE_DECISION') throw new TypeError('Unknown governance commitment evidence kind'); return { kind: 'GOVERNANCE_DECISION' as const, decisionId: text(evidence.decisionId, 'Commitment evidence decision') } })(); return createGovernanceCommitmentEvent({ ...common, kind, actor: parseGovernanceInteractionPartyV3(item.actor), ...(evidence === undefined ? {} : { evidence }) }) }
...
function parseGovernanceUniverseProfileV3(value: unknown): GovernanceUniverseProfile { const item=record(value,'Governance universe profile V3');assertExactKeys(item,['id','institutionId','kind','ownershipModel','expectedBodyKinds',...(item.apexAuthorityBodyId===undefined?[]:['apexAuthorityBodyId']),...(item.executiveAuthorityBodyId===undefined?[]:['executiveAuthorityBodyId']),...(item.basketballOperationsBodyId===undefined?[]:['basketballOperationsBodyId']),'externalOversight'],'Governance universe profile V3');if(typeof item.externalOversight!=='boolean')throw new TypeError('External oversight must be boolean');return createGovernanceUniverseProfile({id:text(item.id,'Profile ID'),institutionId:text(item.institutionId,'Profile institution'),kind:enumValue(item.kind,['PROFESSIONAL_CLUB','NBA_FRANCHISE','WNBA_FRANCHISE','NCAA_PROGRAM','NCAAW_PROGRAM','FEDERATION'],'Profile kind') as never,ownershipModel:enumValue(item.ownershipModel,['PRIVATE','OWNERSHIP_GROUP','MEMBER_OWNED','MULTISPORT_SECTION','UNIVERSITY','FEDERATION'],'Ownership model') as never,expectedBodyKinds:array(item.expectedBodyKinds,'Expected body kinds').map((kind)=>enumValue(kind,GOVERNANCE_BODY_KINDS,'Body kind') as never),...(item.apexAuthorityBodyId===undefined?{}:{apexAuthorityBodyId:text(item.apexAuthorityBodyId,'Apex body')}),...(item.executiveAuthorityBodyId===undefined?{}:{executiveAuthorityBodyId:text(item.executiveAuthorityBodyId,'Executive body')}),...(item.basketballOperationsBodyId===undefined?{}:{basketballOperationsBodyId:text(item.basketballOperationsBodyId,'Basketball body')}),externalOversight:item.externalOversight})}
...
function parseGovernanceDecisionParticipationGrantV3(value: unknown): GovernanceDecisionParticipationGrant { const item=record(value,'Governance decision participation grant V3');assertExactKeys(item,['id','authorityGrantId','bodyId','edgeParticipant','right',...(item.approvalRequirement===undefined?[]:['approvalRequirement'])],'Governance decision participation grant V3');return createGovernanceDecisionParticipationGrant({id:text(item.id,'Participation ID'),authorityGrantId:text(item.authorityGrantId,'Authority grant ID'),bodyId:text(item.bodyId,'Body ID'),edgeParticipant:enumValue(item.edgeParticipant,['DELEGATOR','DELEGATE'],'Edge participant') as never,right:enumValue(item.right,GOVERNANCE_DECISION_RIGHTS,'Decision right') as never,...(item.approvalRequirement===undefined?{}:{approvalRequirement:enumValue(item.approvalRequirement,['ALL_OF'],'Approval requirement') as never})}) }
function parseGovernanceDecisionV3(value: unknown): GovernanceDecision { const item=record(value,'Governance decision V3');assertExactKeys(item,['id','institutionId','decisionType','proposedByBodyId','proposedOn','subject',...(item.source===undefined?[]:['source'])],'Governance decision V3');const subject=parseGovernanceDecisionSubjectV3(item.subject);const source=item.source===undefined?undefined:parseGovernanceDecisionSourceV3(item.source);return createGovernanceDecision({id:text(item.id,'Decision ID'),institutionId:text(item.institutionId,'Decision institution'),decisionType:enumValue(item.decisionType,GOVERNANCE_DECISION_TYPES,'Decision type') as never,proposedByBodyId:text(item.proposedByBodyId,'Decision proposer'),proposedOn:parseGameDate(text(item.proposedOn,'Decision proposedOn')),subject,...(source===undefined?{}:{source})}) }
function parseGovernanceDecisionSubjectV3(value: unknown): GovernanceDecision['subject'] { const item=record(value,'Governance decision subject V3');const kind=text(item.kind,'Decision subject kind');if(kind==='COACH'){assertExactKeys(item,['kind','coachId'],'Coach subject');return {kind,coachId:text(item.coachId,'Coach ID')}}if(kind==='EXECUTIVE'){assertExactKeys(item,['kind','staffId'],'Executive subject');return {kind,staffId:text(item.staffId,'Staff ID')}}if(kind==='BUDGET'){assertExactKeys(item,['kind','scope','referenceId'],'Budget subject');return {kind,scope:enumValue(item.scope,['TEAM','INSTITUTION'],'Budget scope') as never,referenceId:text(item.referenceId,'Budget reference')}}if(kind==='FACILITY'){assertExactKeys(item,['kind','facilityId'],'Facility subject');return {kind,facilityId:text(item.facilityId,'Facility ID')}}if(kind==='ORGANIZATIONAL'){assertExactKeys(item,['kind','institutionId',...(item.bodyId===undefined?[]:['bodyId'])],'Organizational subject');return {kind,institutionId:text(item.institutionId,'Organizational institution'),...(item.bodyId===undefined?{}:{bodyId:text(item.bodyId,'Organizational body')})}}if(kind==='GENERIC'){assertExactKeys(item,['kind','referenceId'],'Generic subject');return {kind,referenceId:text(item.referenceId,'Generic reference')}}throw new TypeError('Unknown governance decision subject kind') }
function parseGovernanceDecisionSourceV3(value: unknown): GovernanceDecision['source'] { const item=record(value,'Governance decision source V3');assertExactKeys(item,['kind','evaluationId'],'Governance decision source V3');if(item.kind!=='MANAGER_EVALUATION')throw new TypeError('Unknown governance decision source kind');return {kind:'MANAGER_EVALUATION',evaluationId:text(item.evaluationId,'Manager evaluation ID')} }
function parseGovernanceDecisionEventV3(value: unknown): GovernanceDecisionEvent { const item=record(value,'Governance decision event V3');assertExactKeys(item,['id','decisionId','kind','bodyId','effectiveOn','authorityGrantIds'],'Governance decision event V3');return createGovernanceDecisionEvent({id:text(item.id,'Decision event ID'),decisionId:text(item.decisionId,'Decision ID'),kind:enumValue(item.kind,GOVERNANCE_DECISION_EVENT_KINDS,'Decision event kind') as never,bodyId:text(item.bodyId,'Event body ID'),effectiveOn:parseGameDate(text(item.effectiveOn,'Event effectiveOn')),authorityGrantIds:array(item.authorityGrantIds,'Event authority grants').map((id)=>text(id,'Authority grant ID'))}) }
...
function values<T extends { readonly id: string }>(value: Readonly<Record<string, T>>): readonly T[] { return Object.values(value).sort((a, b) => a.id.localeCompare(b.id)) }
