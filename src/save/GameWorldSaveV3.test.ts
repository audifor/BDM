...
import { deriveGovernanceFollowUpItems, deriveGovernanceInteractionSignals } from '@/domain/governance'
...
describe('GameWorldSaveV3', () => {
  it('round-trips BG5C commitments, commitment request evidence, agenda references, and legacy defaults', () => {
    const base = createNewGame(), teamId = Object.values(base.teams)[0]!.id
    const institution = createGovernanceInstitution({ id: 'bg5c-institution', universe: 'PROFESSIONAL_CLUB', name: 'BG5C Club', teamIds: [teamId] }), body = createGovernanceBody({ id: 'bg5c-body', institutionId: institution.id, kind: 'BOARD', name: 'Board' })
    const meeting = { id: 'bg5c-meeting', institutionId: institution.id, conveningBodyId: body.id, category: 'ORDINARY' as const, title: 'Commitments', createdOn: base.currentDate }, request = { id: 'bg5c-request', institutionId: institution.id, issuer: { kind: 'BODY' as const, bodyId: body.id }, recipient: { kind: 'ACTOR' as const, actor: { kind: 'COACH' as const, id: base.userCoachId } }, category: 'PERFORMANCE' as const, summary: 'Improve results', origin: { kind: 'STANDALONE' as const } }, commitment = { id: 'bg5c-commitment', institutionId: institution.id, promisor: { kind: 'BODY' as const, bodyId: body.id }, beneficiary: request.recipient, summary: 'Deliver plan', provenance: { kind: 'MEETING' as const, meetingId: meeting.id, agendaItemId: 'bg5c-topic' } }
    const world = updateGameWorld(base, { governanceInstitutions: [institution], governanceBodies: [body], governanceMeetings: [meeting], governanceMeetingEvents: [{ id: '01', meetingId: meeting.id, kind: 'SCHEDULED' as const, effectiveOn: base.currentDate, scheduledFor: base.currentDate }, { id: '02', meetingId: meeting.id, kind: 'HELD' as const, effectiveOn: base.currentDate }], governanceMeetingAgendaItems: [{ id: 'bg5c-topic', meetingId: meeting.id, sequence: 1, subject: { kind: 'INSTITUTIONAL_TOPIC' as const, topic: 'Plan' } }, { id: 'bg5c-agenda', meetingId: meeting.id, sequence: 2, subject: { kind: 'GOVERNANCE_COMMITMENT' as const, commitmentId: commitment.id } }], governanceRequests: [request], governanceRequestEvents: [{ id: '01-request', requestId: request.id, kind: 'ISSUED' as const, effectiveOn: base.currentDate, actor: request.issuer }, { id: '03-request', requestId: request.id, kind: 'FULFILLED' as const, effectiveOn: base.currentDate, actor: request.recipient, evidence: { kind: 'GOVERNANCE_COMMITMENT' as const, commitmentId: commitment.id } }], governanceCommitments: [commitment], governanceCommitmentEvents: [{ id: '01-commitment', commitmentId: commitment.id, kind: 'MADE' as const, effectiveOn: base.currentDate, actor: commitment.promisor }, { id: '02-commitment', commitmentId: commitment.id, kind: 'FULFILLED' as const, effectiveOn: base.currentDate, actor: commitment.beneficiary }] })
    const derivationInput = (value: typeof world) => ({ institutionId: institution.id, asOfDate: value.currentDate, requests: Object.values(value.governanceRequestsById), requestEvents: Object.values(value.governanceRequestEventsById), commitments: Object.values(value.governanceCommitmentsById), commitmentEvents: Object.values(value.governanceCommitmentEventsById) })
    const signalsBefore = deriveGovernanceInteractionSignals(derivationInput(world)), followUpsBefore = deriveGovernanceFollowUpItems(derivationInput(world))
    const saved = serializeGameWorldV3(world, savedAt), loaded = deserializeGameWorldV3(saved)
    expect(deriveGovernanceInteractionSignals(derivationInput(loaded))).toEqual(signalsBefore); expect(deriveGovernanceFollowUpItems(derivationInput(loaded))).toEqual(followUpsBefore)
    expect(loaded.governanceCommitmentsById).toEqual(world.governanceCommitmentsById); expect(loaded.governanceCommitmentEventsById).toEqual(world.governanceCommitmentEventsById); expect((loaded.governanceRequestEventsById['03-request']! as Extract<import('@/domain/governance').GovernanceRequestEvent, { kind: 'FULFILLED' }>).evidence).toEqual({ kind: 'GOVERNANCE_COMMITMENT', commitmentId: commitment.id }); expect(loaded.governanceMeetingAgendaItemsById['bg5c-agenda']!.subject).toEqual({ kind: 'GOVERNANCE_COMMITMENT', commitmentId: commitment.id })
    const legacy = structuredClone(saved), runtime = legacy.payload.staffCareerRuntime as Record<string, unknown>; delete runtime.governanceCommitments; delete runtime.governanceCommitmentEvents; runtime.governanceMeetingAgendaItems = [runtime.governanceMeetingAgendaItems as unknown as Record<string, unknown>[]][0]!.filter((item) => (item.subject as Record<string, unknown>).kind !== 'GOVERNANCE_COMMITMENT'); runtime.governanceRequestEvents = [runtime.governanceRequestEvents as unknown as Record<string, unknown>[]][0]!.filter((item) => item.kind !== 'FULFILLED')
    const legacyLoaded = deserializeGameWorldV3(legacy); expect(legacyLoaded.governanceCommitmentsById).toEqual({}); expect(legacyLoaded.governanceCommitmentEventsById).toEqual({})
  })
  it('rejects malformed and world-invalid BG5C V3 commitment payloads', () => {
    const base = createNewGame(), teamId = Object.values(base.teams)[0]!.id, institution = createGovernanceInstitution({ id: 'bg5c-invalid-institution', universe: 'PROFESSIONAL_CLUB', name: 'BG5C', teamIds: [teamId] }), body = createGovernanceBody({ id: 'bg5c-invalid-body', institutionId: institution.id, kind: 'BOARD', name: 'Board' }), commitment = { id: 'bg5c-invalid-commitment', institutionId: institution.id, promisor: { kind: 'BODY' as const, bodyId: body.id }, beneficiary: { kind: 'ACTOR' as const, actor: { kind: 'COACH' as const, id: base.userCoachId } }, summary: 'Plan', provenance: { kind: 'STANDALONE' as const } }, world = updateGameWorld(base, { governanceInstitutions: [institution], governanceBodies: [body], governanceCommitments: [commitment], governanceCommitmentEvents: [{ id: '01', commitmentId: commitment.id, kind: 'MADE' as const, effectiveOn: base.currentDate, actor: commitment.promisor }] }), saved = structuredClone(serializeGameWorldV3(world, savedAt)), runtime = saved.payload.staffCareerRuntime as Record<string, unknown>
    runtime.governanceCommitments = [{ ...commitment, provenance: { kind: 'UNKNOWN' } }]; expect(() => deserializeGameWorldV3(saved)).toThrow()
    const malformed = structuredClone(serializeGameWorldV3(world, savedAt)), malformedRuntime = malformed.payload.staffCareerRuntime as Record<string, unknown>; malformedRuntime.governanceCommitmentEvents = [{ id: 'bad', commitmentId: commitment.id, kind: 'SUPERSEDED', effectiveOn: base.currentDate }]; expect(() => deserializeGameWorldV3(malformed)).toThrow()
  })
  ...
  it('round-trips generic BG4 decision evidence and loads a V3 payload without BG4 fields', () => {
    ...
    const world = updateGameWorld(base, { governanceInstitutions: [institution], governanceBodies: [owner, board], governanceAuthorityGrants: [grant], governanceManagerEvaluationPeriods: [period], governanceManagerEvaluations: [evaluation], governanceDecisionParticipationGrants: [participation], governanceDecisions: [decision], governanceDecisionEvents: [event] }), saved = serializeGameWorldV3(world, savedAt), loaded = deserializeGameWorldV3(saved)
    expect(loaded.governanceDecisionsById).toEqual(world.governanceDecisionsById); expect(loaded.governanceDecisionParticipationGrantsById).toEqual(world.governanceDecisionParticipationGrantsById); expect(loaded.governanceDecisionEventsById).toEqual(world.governanceDecisionEventsById)
    const legacy = structuredClone(saved), runtime = legacy.payload.staffCareerRuntime as Record<string, unknown>; delete runtime.governanceDecisionParticipationGrants; delete runtime.governanceDecisions; delete runtime.governanceDecisionEvents
    const oldLoaded = deserializeGameWorldV3(legacy); expect(oldLoaded.governanceDecisionParticipationGrantsById).toEqual({}); expect(oldLoaded.governanceDecisionsById).toEqual({}); expect(oldLoaded.governanceDecisionEventsById).toEqual({})
  })
