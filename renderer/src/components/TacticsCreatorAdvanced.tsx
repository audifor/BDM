...
export default function TacticsCreator({ clubId, onSaveCustomPlay, theme = "dark" }: TacticsCreatorProps) {
  const isLight = theme === "light";
  const THEME = {
    bg: isLight ? "var(--surface-secondary)" : "#1e293b",
    panelBg: isLight ? "var(--surface-primary)" : "#1e293b",
    border: isLight ? "rgba(15, 23, 42, 0.12)" : "#334155",
    text: isLight ? "#0f172a" : "#e2e8f0",
    muted: isLight ? "#64748b" : "#94a3b8",
    shadow: isLight ? "0 0 30px rgba(15, 23, 42, 0.12)" : "0 0 40px rgba(0,0,0,0.5)",
    cardBg: isLight ? "rgba(255, 255, 255, 0.9)" : "#0f172a",
    subtleBg: isLight ? "rgba(15, 23, 42, 0.04)" : "rgba(255,255,255,0.03)",
    previewStroke: isLight ? "rgba(15, 23, 42, 0.35)" : "rgba(255,255,255,0.5)",
    courtLines: isLight ? "#94a3b8" : "#64748b",
    moveColor: isLight ? "#0f172a" : "#ffffff",
    accentText: "#0f172a",
    passColor: "#fbbf24",
    dribbleColor: "#38bdf8",
    screenColor: "#ef4444",
  ...
  const toolBtnStyle = (active: boolean, color?: string) => ({
    width: 50,
    height: 50,
    borderRadius: 6,
    border: active ? `2px solid ${color || THEME.text}` : "1px solid transparent",
    background: active
  ...
  const ToolBtn = ({ icon, label, active, onClick, color }: any) => (
    <button onClick={onClick} style={toolBtnStyle(active, color)}>
      {icon}
      <span style={{ fontSize: "0.6rem", fontWeight: active ? "bold" : "normal" }}>{label}</span>
    </button>
  );

  const timelineControlBtn = {
    width: 40,
    height: 40,
    borderRadius: "50%",
  ...
  const modalOverlay: any = {
    position: "absolute",
    inset: 0,
    background: isLight ? "rgba(226, 232, 240, 0.8)" : "rgba(0,0,0,0.8)",
    zIndex: 200,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', height:'100%', width:'100%', gap:10, fontFamily:'sans-serif', overflow:'hidden', position:'relative'}}>
          ...
          <div style={modalOverlay}>
              <div style={modalContent}>
                  ...
                  <div style={listContainer}>
                      {savedPlays.length === 0 ? <p style={{color: THEME.muted}}>No hay jugadas guardadas.</p> : savedPlays.map(play => (
                          <div key={play.id} onClick={()=>{ setFrames(play.frames); setPlayName(play.name); setCurrentFrameIndex(0); setModalMode('none'); }} style={listItem}>
                              <div><div style={{color: THEME.text, fontWeight:'bold'}}>{play.name}</div><div style={{color: THEME.muted, fontSize:'0.8rem'}}>{play.date}</div></div>
      ...
      {modalMode === 'createPB' && (
          <div style={modalOverlay}>
              <div style={modalContent}>
                  ...
                  <input type="text" placeholder="" value={newPbName} onChange={e=>setNewPbName(e.target.value)} style={inputStyle} />
      ...
      {modalMode === 'editPB' && (
          <div style={modalOverlay}>
              <div style={{...modalContent, width: 600}}>
                  ...
                  {!selectedPbForEdit ? (
                      <div style={listContainer}>
                          {playbooks.length === 0 ? <p style={{color: THEME.muted}}>No tienes playbooks.</p> : playbooks.map(pb => (
                              <div key={pb.id} onClick={()=>setSelectedPbForEdit(pb)} style={listItem}>
                                  <div style={{display:'flex', alignItems:'center', gap:10}}><Book size={20} color="#38bdf8"/><div><div style={{color: THEME.text, fontWeight:'bold'}}>{pb.name}</div><div style={{color: THEME.muted, fontSize:'0.8rem'}}>{pb.playIds.length} jugadas</div></div></div>
                      ...
                      <div>
                          ...
                          <div style={{...listContainer, maxHeight:250}}>
                              {selectedPbForEdit.playIds.length === 0 ? <p style={{color: THEME.muted}}>Vacío.</p> : selectedPbForEdit.playIds.map(pid => {
                                    const play = savedPlays.find(p => p.id === pid);
                                    if(!play) return null;
                                    return (
                                        <div key={pid} style={{...listItem, cursor:'default'}}>
                                            <span style={{color: THEME.text}}>{play.name}</span>
      ...
      {/* HEADER */}
      <div style={{flexShrink:0, padding:'8px 15px', background: THEME.panelBg, borderRadius:8, border:`1px solid ${THEME.border}`, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <div style={{display:'flex', alignItems:'center', gap:15}}>
             <input type="text" value={playName} onChange={(e) => setPlayName(e.target.value)} style={{background:'transparent', border:'none', color: THEME.text, fontSize:'1.1rem', fontWeight:'bold', borderBottom:`1px solid ${THEME.border}`, paddingBottom:2, outline:'none', width:200}}/>
             <span style={{color: THEME.muted, fontSize:'0.75rem'}}>Frame <b style={{color: THEME.passColor}}>{displayIndex + 1}/{frames.length}</b></span>
        </div>
        <div style={{display:'flex', gap:8}}>
             ...
             <div style={{position:'relative'}}>
                 ...
                 {showAddToPbDropdown && (
                     <div style={{position:'absolute', top:'100%', right:0, marginTop:5, background: THEME.panelBg, border:`1px solid ${THEME.border}`, borderRadius:6, width:200, zIndex:100, boxShadow: THEME.shadow}}>
                         {playbooks.length === 0 ? <div style={{padding:10, color: THEME.muted, fontSize:'0.8rem'}}>No hay playbooks</div> : playbooks.map(pb => (
                                <div key={pb.id} onClick={()=>addCurrentPlayToPlaybook(pb.id)} style={{padding:'8px 12px', color: THEME.text, cursor:'pointer', borderBottom:`1px solid ${THEME.border}`, fontSize:'0.85rem'}}>{pb.name}</div>
                            ))}
                     </div>
                 )}
             </div>
      ...
      <div style={{flex:1, display:'flex', gap:10, minHeight:0, overflow:'hidden'}}>
          {/* TOOLBAR */}
          <div style={{width: 70, background: THEME.panelBg, borderRadius:8, border:`1px solid ${THEME.border}`, display:'flex', flexDirection:'column', alignItems:'center', padding:'15px 0', gap: 8, overflowY:'auto'}}>
             <ToolBtn icon={<MousePointer2 size={20}/>} label="Select" active={activeTool==='select'} onClick={()=>setActiveTool('select')} />
             <div style={{width:'60%', height:1, background: THEME.border, margin:'5px 0'}}></div>
             ...
             <div style={{marginTop:'auto'}}><button onClick={() => updateFrame({ paths: [] })} style={{background:'transparent', border:'none', color: THEME.muted, cursor:'pointer'}} title="Borrar"><Eraser size={18}/></button></div>
          </div>

          {/* CANVAS */}
          <div style={{flex:1, background: THEME.cardBg, borderRadius:8, border:`1px solid ${THEME.border}`, display:'flex', alignItems: 'center', justifyContent: 'center', overflow:'hidden', position:'relative'}}>
             ...
             <svg 
                ...
                style={{maxHeight:'95%', maxWidth:'95%', aspectRatio:'50/47', background: THEME.bg, border:`2px solid ${THEME.border}`, cursor: activeTool==='select'?(dragging?'grabbing':'default'):'crosshair', boxShadow: THEME.shadow, touchAction:'none'}}
             ...
             >
                ...
                <g stroke={THEME.courtLines} strokeWidth="2" fill="none">
                    ...
                    <line x1="220" y1="40" x2="280" y2="40" strokeWidth="3" stroke={THEME.text}/> <circle cx="250" cy="52.5" r="7.5" stroke="#f97316" strokeWidth="2"/>
                ...
                {isDrawing && currentPathPoints.length > 0 && <path d={activeTool==='pass'?pointsToStraightPath([currentPathPoints[0], currentPathPoints[currentPathPoints.length-1]]) : pointsToCurvedPath(currentPathPoints)} stroke={THEME.previewStroke} strokeWidth="3" fill="none" />}
          ...
          {/* RESUMEN LATERAL */}
          <div style={{width: 180, background: THEME.panelBg, borderLeft:`1px solid ${THEME.border}`, padding:15, overflowY:'auto'}}>
              <div style={{fontSize:'0.75rem', fontWeight:'bold', color: THEME.muted, marginBottom:15, display:'flex', alignItems:'center', gap:5, borderBottom:`1px solid ${THEME.border}`, paddingBottom:10}}><ListChecks size={16}/> RESUMEN</div>
              {currentFrame.paths.length === 0 && <span style={{fontSize:'0.7rem', color: THEME.muted}}>Sin acciones</span>}
              <div style={{display:'flex', flexDirection:'column', gap:8}}>
                {currentFrame.paths.map((p,i) => {
                    const pl = currentFrame.players.find(x=>x.id===p.linkedId);
                    return (<div key={i} style={{fontSize:'0.75rem', color: THEME.text, background: THEME.subtleBg, padding:'8px', borderRadius:6, borderLeft:`3px solid ${p.type==='pass'?THEME.passColor:(p.type==='dribble'?THEME.dribbleColor:(p.type==='screen'?THEME.screenColor:THEME.moveColor))}`}}>
                        <div style={{fontWeight:'bold', color: THEME.muted, fontSize:'0.65rem', marginBottom:2}}>{pl ? `JUGADOR ${pl.label}` : 'JUGADOR'}</div>
      ...
      {/* TIMELINE */}
      <div style={{flexShrink: 0, height: 80, background: THEME.panelBg, borderRadius:8, border:`1px solid ${THEME.border}`, display:'flex', alignItems:'center', padding:'0 20px', gap:20}}>
          ...
          <div style={{width:1, height:'50%', background: THEME.border}}></div>
          <div style={{flex:1, display:'flex', gap:10, overflowX:'auto', paddingBottom:5, alignItems:'center'}}>
             {frames.map((_, index) => (
                <div key={index} onClick={() => { setIsPlaying(false); setCurrentFrameIndex(index); }} style={{minWidth: 60, height: 60, borderRadius: 8, border: displayIndex === index ? `2px solid ${THEME.dribbleColor}` : `1px solid ${THEME.border}`, background: displayIndex === index ? 'rgba(56, 189, 248, 0.1)' : THEME.cardBg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', cursor:'pointer', position:'relative'}}>
                   <span style={{fontSize:'1rem', fontWeight:'bold', color: displayIndex === index ? THEME.dribbleColor : THEME.muted}}>{index + 1}</span>
             ...
             <button onClick={addFrame} style={{minWidth: 60, height: 60, borderRadius: 8, border:`1px dashed ${THEME.muted}`, background:'transparent', color: THEME.muted, cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:5}}><Plus size={20}/><span style={{fontSize:'0.6rem'}}>AÑADIR</span></button>
          </div>
      </div>
    </div>
  );
