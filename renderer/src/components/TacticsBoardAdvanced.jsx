...
export default function TacticsBoardAdvanced({
  teamId,
  roster,
  tacticalRoles,
  onRolesChange,
  ...
  const mutedText = isLight ? "#94a3b8" : "#475569";
  const activeAccent = isLight ? "#f97316" : "#a5f3fc";
  const tabInactiveBorder = isLight ? "rgba(15, 23, 42, 0.18)" : "#334155";
  const tabActiveBg = isLight ? "rgba(255, 255, 255, 0.9)" : "#1e293b";
  const tabActiveColor = isLight ? "#b45309" : "#a5f3fc";
  ...
  const saveStarters = (nextStarters) => {
    ...
    if (typeof onStartersChange === "function") {
      onStartersChange(payload);
    }
  };
  ...
  const handleDropOnCourt = (event, positionKey) => {
    ...
    if (dragSource === "roster") {
      ...
      if (currentPlayerInSlot) {
        setBench((prev) => [...prev, currentPlayerInSlot].sort((a, b) => a.name.localeCompare(b.name)));
      }
  ...
  const handleDropOnRoster = (event) => {
    ...
    if (dragSource && dragSource !== "roster" && draggedPlayer) {
      setBench((prev) => [...prev, draggedPlayer].sort((a, b) => a.name.localeCompare(b.name)));
  ...
  const tabBtnStyle = (isActive) => ({
    ...
    color: isActive ? tabActiveColor : mutedText,
    border: "none",
    cursor: "pointer",
    fontWeight: "bold",
    fontSize: "0.75rem",
    borderBottom: isActive ? `2px solid ${activeAccent}` : `1px solid ${tabInactiveBorder}`,
  });
        ...
        <div className="tactics-board-panel-body">
          {bench.map((p) => (
            ...
            >
              ...
              <GripVertical size={16} color={mutedText} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="tactics-board-court">
        <div className="tactics-board-court-header">
          ...
          <button
            ...
            onClick={() => {
              ...
              setBench((prev) => [...prev, ...Object.values(starters).filter(Boolean)].sort((a, b) => a.name.localeCompare(b.name)));
              setStarters(empty);
              saveStarters(empty);
            }}
          >
        ...
        <div className="tactics-board-court-surface">
          ...
          {SLOT_IDS.map((slotId) => {
            ...
            const suitabilityColor = player ? getSuitabilityColor(player.position, slotId) : mutedText;
            const dutyColor =
              ...
              >
                <div className="tactics-board-slot-ring" style={{ borderColor: suitabilityColor }}>
                  {player ? (
                    ...
                    >
                      <User size={38} color={isLight ? "#475569" : "#cbd5e1"} />
                      {penalty && (
