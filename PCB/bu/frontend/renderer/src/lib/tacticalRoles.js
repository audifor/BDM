...
export const getPlayerOverall = (player) => {
  ...
  const fallback = Number(player?.current_ability || player?.rating || 50);
  const normalizedFallback = normalizeAttrValue(fallback);
  return normalizedFallback !== null ? normalizedFallback : 50;
};
