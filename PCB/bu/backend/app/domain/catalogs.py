...
def load_catalogs() -> Dict[str, CatalogValue]:
    ...
    origin_pairs = re.findall(r"\|\s*\*\*`(CTX_\d{2})`\*\*\s*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", origins_text)
    ...
    trait_pairs = re.findall(r"\|\s*`(TRA_[A-Z]+_\d+)`\s*\|[^|]*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", traits_text)
    ...
    perk_pairs = re.findall(r"\|\s*`(PRK_[A-Z]+_\d+)`\s*\|[^|]*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", perks_text)
    perks = [p[0] for p in perk_pairs] or re.findall(r"PRK_[A-Z]+_\d+", perks_text)
    perk_labels = {pid: _clean_label(label) for pid, label, _ in perk_pairs}
    ...
    mental_pairs = re.findall(r"\|\s*\*\*(\d+)\*\*\s*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|\s*([^|]+)\|\s*([^|]+)\|", mental_text)
