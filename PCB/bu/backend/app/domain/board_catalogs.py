...
def load_board_catalogs() -> Dict[str, object]:
    ...
    for line in roles_text.splitlines():
        if "BRD_" not in line:
            continue
        match = re.search(r"`(BRD_\d+)`\s*\|\s*\*\*([A-Z_]+)\*\*\s*\|\s*\*\*([^|]+)\*\*", line)
        if not match:
            continue
        role_id, category, role_name = match.group(1), match.group(2), _clean_label(match.group(3))
        roles.append({"id": role_id, "category": category, "role": role_name})

    profiles: List[Dict[str, str]] = []
    profile_pairs = re.findall(r"\|\s*\*\*(\d+)\*\*\s*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|\s*([^|]+)\|", pers_text)
