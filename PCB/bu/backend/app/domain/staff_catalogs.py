...
def load_staff_catalogs() -> Dict[str, object]:
    ...
    trait_pairs_s = re.findall(r"\|\s*`(STR_[A-Z]_\d{2})`\s*\|[^|]*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", traits_s_text)
    trait_pairs_m = re.findall(r"\|\s*`(STR_[A-Z]_\d{2})`\s*\|[^|]*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", traits_m_text)
    ...
    perk_pairs_s = re.findall(r"\|\s*`(SPR_[A-Z]_\d{2})`\s*\|[^|]*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", perks_s_text)
    perk_pairs_m = re.findall(r"\|\s*`(SPR_[A-Z]_\d{2})`\s*\|[^|]*\|\s*\*\*([^|]+)\*\*\s*\|\s*([^|]+)\|", perks_m_text)
    ...
    for line in roles_text.splitlines():
        if "STF_" not in line:
            continue
        role_match = re.search(r"`(STF_\d+)`\s*\|\s*([A-Z_]+)\s*\|\s*\*\*([^|]+)\*\*", line)
        if not role_match:
            continue
        role_id, department, role_name = role_match.group(1), role_match.group(2), _clean_label(role_match.group(3))
