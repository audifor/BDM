...
def _generate_player_attributes(attributes: List[str], categories: Dict[str, str], pos: str) -> Dict[str, int]:
...
def generate_player(seed: int | None = None, pos: str | None = None) -> Dict[str, Any]:
    if seed is not None:
        random.seed(seed)
    ...
    attr_map = _generate_player_attributes(attributes, attribute_categories, pos)

    scouting_tier = random.choices([1, 2, 3, 4, 5, 6], weights=[8, 20, 26, 22, 16, 8], k=1)[0]
    height_cm = random.randint(178, 218)
