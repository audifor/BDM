...
def _normalize_contract_type(value: str | None) -> str:
    ...
    if raw in {"beca", "scholarship", "amateur", "non_pro", "non-pro"}:
        return "scholarship"
...
def _apply_contract_type(contract: Dict[str, object], contract_type: str | None) -> Dict[str, object]:
    normalized = _normalize_contract_type(contract_type)
    contract["type"] = normalized
    contract["is_professional"] = normalized not in {"scholarship"}
    if normalized == "scholarship":
        years = max(1, int(contract.get("years") or 1))
        contract["salary"] = 0
        contract["yearly_salary"] = [0 for _ in range(years)]
        contract["guaranteed_pct"] = 0
        contract["bonuses"] = []
        contract["bonuses_detail"] = []
    return contract
...
def create_contract_data(
    ...
    return _apply_contract_type(contract, contract_type)
...
def apply_contract_type(contract: Dict[str, object], contract_type: str | None) -> Dict[str, object]:
    return _apply_contract_type(contract, contract_type)
