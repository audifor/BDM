...
1. **`headCoach` role identity** (§6.4): the canonical Head Coach role is carried by `StaffProfile`; `Coach` is only its gameplay/RPG facade. Supporting responsibility contracts may retain their existing `eligibleParticipant: 'coach'` marker until those contracts are migrated, but they must resolve through the Coach facade's `staffProfileId` and never create a parallel `Person` profile.
