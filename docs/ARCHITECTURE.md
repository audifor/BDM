...
`GameWorld.memoriesById` stores structured, observer-owned memories, separate from the canonical event/history that caused them. A memory has a semantic key for deterministic deduplication, canonical entity references, minimal context, perspective valence, importance, current intensity, tags, optional relationship impact and an optional permanent flag. Coaches, players, staff and teams can own memories; the same source event can therefore have conflicting perspectives.

The Memory domain owns semantic queries for memories by observer, related entity, team, type, valence, importance and shared history. The Memory Engine is the mutation boundary: it records a memory once, applies any relationship impact through the existing relationship event API once, and decays non-permanent intensity at the monthly calendar checkpoint without deleting historical records. Personality-aware interpretation is available as a deterministic helper, while reputation remains a separate aggregate system.

V1 records coach appointments/dismissals and season championships: the winning team and its coach retain permanent championship memories. Save V1 persists memories and legacy saves receive an empty collection. Narrative construction, recurring relationship patterns, player-role memories, interview text and dynamic story arcs are intentionally reserved for Hito 063 and later.
